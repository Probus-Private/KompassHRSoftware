...
{
    public class ESS_HC_MDMappingByHRController : Controller
    {
        ...
        // GET: Setting/ ESS_HC_MDMappingByHR
        #region Main View
        public ActionResult ESS_HC_MDMappingByHR(Manpower_HC_MDMapping Manpower_HC_MDMapping)
        {
            try
            {
                ...
                ViewBag.GetHC_MDMappingData = "";

                if (Manpower_HC_MDMapping.CmpId > 0)
                {
                    DynamicParameters paramBU = new DynamicParameters();
                    paramBU.Add("@p_employeeid", Session["EmployeeId"]);
                    paramBU.Add("@p_CmpId", Manpower_HC_MDMapping.CmpId);
                    ...
                    paramHC.Add("@p_Process", "LineManpowerHC_MDList");
                    paramHC.Add("@p_CmpId", Manpower_HC_MDMapping.CmpId);
                    paramHC.Add("@p_BranchId", Manpower_HC_MDMapping.BranchId);
                    paramHC.Add("@p_Date", Manpower_HC_MDMapping.Date);
                    var lineManpowerList = DapperORM.ExecuteSP<dynamic>("sp_Get_LineManPowerCatgeoryFor_HC_MD", paramHC).ToList();

                    ViewBag.GetLineandManpowerCategoryList = lineManpowerList;
                    if (lineManpowerList == null || !lineManpowerList.Any())
                    {
                        var GetCompanyName = "select CompanyName from Mas_CompanyProfile where CompanyId = '" + Manpower_HC_MDMapping.CmpId + "' AND Deactivate = 0";
                        var CompanyName = DapperORM.DynamicQuerySingle(GetCompanyName);

                        var GetBranchName = "select BranchName from Mas_Branch where  BranchId = '" + Manpower_HC_MDMapping.BranchId + "' AND Deactivate = 0";
                    ...
                    TempData["Date"] = Manpower_HC_MDMapping.Date.ToString("yyyy-MM-dd");

                    if (Manpower_HC_MDMapping.HC_MDMappingMasterID_Encrypted != null)
                    {
                        ViewBag.AddUpdateTitle = "Update";

                        DynamicParameters param2 = new DynamicParameters();
                        param2.Add("@p_HC_MDMappingMasterID_Encrypted", Manpower_HC_MDMapping.HC_MDMappingMasterID_Encrypted);
                        param2.Add("@p_EmployeeId", Session["EmployeeId"]);
                        Manpower_HC_MDMapping = DapperORM.ReturnList<Manpower_HC_MDMapping>("sp_List_Manpower_HC_MDMapping", param2).FirstOrDefault();
                        if (Manpower_HC_MDMapping != null)
                        {
                            TempData["Date"] = Manpower_HC_MDMapping.Date.ToString("yyyy-MM-dd");
                        }
                    }
                ...
                return View(Manpower_HC_MDMapping);
            }
            catch (Exception ex)
            {
        ...
        #region ESS_HC_MDMappingByHR bulk Main View
        [HttpPost]
        public ActionResult ESS_HC_MDMappingByHR(HttpPostedFileBase AttachFile, Manpower_HC_MDMapping obj)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                    ...
                    using (var xlWorkbook = new XLWorkbook(filePath))
                    {
                        ...
                        while (!worksheet.Cell(validateRow, 2).IsEmpty())
                        {
                            ...
                            if (string.IsNullOrEmpty(headCount))
                            {
                                ...
                                return RedirectToAction("ESS_HC_MDMappingByHR");
                            }

                            string monthDays = worksheet.Cell(validateRow, 4).GetString().Trim();
                            if (string.IsNullOrEmpty(monthDays))
                            {
                                TempData["Message"] = $"Fill in missing information in the MonthDays column at row {validateRow}.";
                                TempData["Title"] = "Empty MonthDays Column Detected in Excel Sheet";
                                TempData["Icon"] = "error";
                                System.IO.File.Delete(filePath);
                                return RedirectToAction("ESS_HC_MDMappingByHR");
                            }
                        ...
                        var typedList = new List<Manpower_HC_MDMapping>();
                        foreach (var rowDict in excelDataList)
                        {
                            int hcMappingDetailsId = 0;
                            if (rowDict.ContainsKey("HC_MDMappingDetailsID") &&
                                rowDict["HC_MDMappingDetailsID"] != null &&
                                !string.IsNullOrWhiteSpace(rowDict["HC_MDMappingDetailsID"].ToString()))
                            {
                                int.TryParse(rowDict["HC_MDMappingDetailsID"].ToString(), out hcMappingDetailsId);
                            }
                            ...
                            typedList.Add(new Manpower_HC_MDMapping
                            {
                                LineName = lineName,
                                ...
                                MonthDays = rowDict.ContainsKey("MonthDays") && !string.IsNullOrWhiteSpace(rowDict["MonthDays"]?.ToString())
                                                ? Convert.ToSingle(rowDict["MonthDays"]) : 0,
                                LineId = lineId,
                                KPISubCategoryId = kpiSubCategoryId,
                                HC_MDMappingDetailsID = hcMappingDetailsId
                            });
                        }

                        // --- Step 5: Assign typed list to ViewBag ---
                        ViewBag.GetLineandManpowerCategoryList = typedList;
                ...
                else
                {
                    if (obj.CmpId != 0 && obj.Date != DateTime.MinValue)
                    {
                        ...
                        paramList.Add("@p_Process", "LineManpowerHC_MDList");
                        paramList.Add("@p_CmpId", obj.CmpId);
                        paramList.Add("@p_BranchId", obj.BranchId);
                        paramList.Add("@p_Date", obj.Date);
                        var lineManpowerList = DapperORM.ExecuteSP<dynamic>("sp_Get_LineManPowerCatgeoryFor_HC_MD", paramList).ToList();
                    ...
                    {
                        ViewBag.GetLineandManpowerCategoryList = new List<Manpower_HC_MDMapping>(); // Empty list
                    }
                }
        ...
        public ActionResult IsHC_MDMappingExists(string CmpId, string BranchId, DateTime Date, string HC_MDMappingMasterID_Encrypted)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                using (SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString))
                {
                    ...
                    param.Add("@p_HC_MDMappingMasterID_Encrypted", HC_MDMappingMasterID_Encrypted);
                    param.Add("@p_CmpId", CmpId);
                    param.Add("@p_BranchId", BranchId);
                    param.Add("@p_Date", Date);
                    param.Add("@p_msg", dbType: DbType.String, direction: ParameterDirection.Output, size: 500);
                    param.Add("@p_Icon", dbType: DbType.String, direction: ParameterDirection.Output, size: 500);
                    var Result = DapperORM.ExecuteReturn("sp_SUD_Manpower_HC_MDMapping", param);
        ...
        #region SaveUpdate
        [HttpPost]
        public ActionResult SaveUpadte(List<Manpower_HC_MDMapping> HC_MDMapping, string CmpId, string BranchId, DateTime Date, string HC_MDMappingMasterID_Encrypted)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                var GetHC_MDMappingMasterID = "SELECT ISNULL(HC_MDMappingMasterID,0) AS HC_MDMappingMasterID " +
                                                  "FROM Manpower_HC_MDMappingMaster WHERE CmpId = '" + CmpId + "' " +
                                                  "AND BranchId = '" + BranchId + "'AND Date = '" + Date + "' AND Deactivate = 0";

                var HC_MDMappingMasterID = DapperORM.DynamicQuerySingle(GetHC_MDMappingMasterID);
                double HC_MDMappingMasterId = 0;

                if (HC_MDMappingMasterID != null)
                {
                    HC_MDMappingMasterId = (double)HC_MDMappingMasterID.HC_MDMappingMasterID;
                }
                string masterId = "";

                if (HC_MDMappingMasterId == 0 || HC_MDMappingMasterId == null)
                {
                    var param = new DynamicParameters();
                    param.Add("@p_process", string.IsNullOrEmpty(HC_MDMappingMasterID_Encrypted) ? "Save" : "Update");
                    param.Add("@p_HC_MDMappingMasterID_Encrypted", HC_MDMappingMasterID_Encrypted);
                    param.Add("@p_CmpId", CmpId);
                    param.Add("@p_BranchId", BranchId);
                    param.Add("@p_Date", Date);
                    ...
                    var data = DapperORM.ExecuteReturn("sp_SUD_Manpower_HC_MDMapping", param);
                ...
                {
                    masterId = HC_MDMappingMasterId.ToString();
                }

                for (var i = 0; i < HC_MDMapping.Count; i++)
                {
                    if (HC_MDMapping[i].HC_MDMappingDetailsID == 0)
                    {
                        string qry = "INSERT INTO dbo.Manpower_HC_MDMappingDetails " +
                                       "(HC_MDMappingMasterID, LineId, ManpowerCategoryId, " +
                                       "HeadCount, MonthDays ,Deactivate, CreatedBy, CreatedDate, MachineName)" +
                                       "VALUES ('" + masterId + "', " +
                                       "'" + HC_MDMapping[i].LineId + "', " +
                                       "'" + HC_MDMapping[i].KPISubCategoryId + "', " +
                                       "'" + HC_MDMapping[i].HeadCount + "', " +
                                       "'" + HC_MDMapping[i].MonthDays + "', " +
                    ...
                    {
                        string qry = "UPDATE dbo.Manpower_HC_MDMappingDetails " +
                                     "SET HeadCount = '" + HC_MDMapping[i].HeadCount + "', " +
                                     "MonthDays = '" + HC_MDMapping[i].MonthDays + "', " +
                                     "ModifiedBy = '" + Session["EmployeeName"] + "', " +
                                     "ModifiedDate = GETDATE(), " +
                                     "MachineName = '" + Dns.GetHostName() + "' " +
                                     "WHERE HC_MDMappingDetailsID = '" + HC_MDMapping[i].HC_MDMappingDetailsID + "' " +
                                     "AND LineId='" + HC_MDMapping[i].LineId + "' " +
                                     "AND ManpowerCategoryId = '" + HC_MDMapping[i].KPISubCategoryId + "';";
        ...
        public ActionResult GetList()
        {
            try
            {
                ...
                param.Add("@p_HC_MDMappingMasterID_Encrypted", "List");
                param.Add("@p_EmployeeId", Session["EmployeeId"]);
                var data = DapperORM.DynamicList("sp_List_Manpower_HC_MDMapping", param);
                ViewBag.GetHC_MDMappingData = data;
        ...
        [HttpGet]
        public ActionResult ShowLine_ManpowerCategory_HeadCountList(DateTime Date, int? CmpId, int? BranchId)
        {
            if (Session["EmployeeId"] == null)
            {
                return RedirectToAction("Login", "Login", new { Area = "" });
            ...
            paramH.Add("@p_Process", "LineManpowerHC_MDList");
            paramH.Add("@p_CmpId", CmpId);
            paramH.Add("@p_BranchId", BranchId);
            paramH.Add("@p_Date", Date);
            var data = DapperORM.ExecuteSP<dynamic>("sp_Get_LineManPowerCatgeoryFor_HC_MD", paramH).ToList();
        ...
        #region Delete
        public ActionResult Delete(double HC_MDMappingMasterID)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                param.Add("@p_HC_MDMappingMasterID", HC_MDMappingMasterID);
                ...
                TempData["Message"] = param.Get<string>("@p_msg");
                TempData["Icon"] = param.Get<string>("@p_Icon");
                return RedirectToAction("GetList", "ESS_HC_MDMappingByHR");
            }
            catch (Exception ex)
            {
                Session["GetErrorMessage"] = ex.Message;
        ...
        public ActionResult DownloadExcelFile(double? CmpId, double? BranchId, DateTime Date)
        {
            try
            {
                ...
                var data = DapperORM.ExecuteSP<dynamic>("sp_Get_LineManPowerCatgeoryFor_HC_MD", paramHC).ToList();
                ...
                using (var workbook = new XLWorkbook())
                {
                    ...
                    worksheet.Cell(1, 1).Value = "Line Manpower Category HC_MD Mapping";
                    ...
                    var hiddenColumns = new List<string>
            {
                "LineId",
                "KPISubCategoryId",
                "HC_MDMappingMasterID",
                "HC_MDMappingDetailsID"
            };
