    ...
    public class ESS_Account_ClaimsSettlementController : Controller
    {
        ...
        public ActionResult Disbursement(int? DocId, string Encrypted, string Status, string Remark, string Origin)
        {
            try
            {
                ...
                {
                    DapperORM.DynamicQuerySingle("update Claim_Advance set  ModifiedBy='" + Session["EmployeeName"] + "',ModifiedDate=GETDATE() ,AdvanceClaimAccountBy='" + Session["EmployeeId"] + "',AdvanceClaimAccountDate= GETDATE(),,IsClaimDisbursment=1   where AdvanceClaimId_Encrypted='" + Encrypted + "'");
                }
