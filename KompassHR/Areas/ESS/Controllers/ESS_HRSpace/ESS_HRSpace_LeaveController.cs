...
{
    public class ESS_HRSpace_LeaveController : Controller
    {
     
            DynamicParameters param = new DynamicParameters();
            SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
            // GET: ESS/ESS_HRSpace_Leave
            #region ESS_HRSpace_Leave Main View 
            [HttpGet]
            public ActionResult ESS_HRSpace_Leave(Leave_Master leave)
            {
                try
                {
                    ...
                    int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 917;
                    ...
                    ViewBag.GetLeaveBalance = "";
                     ViewBag.GetLeaveYear = "";
                        ...
                        {
                        var GetCmpID = "select CmpID from Mas_Employee where EmployeeId='"+ leave.LeaveMasterEmployeeId + "'";
                        var CmpID = DapperORM.QuerySingle(GetCmpID);
                         var CmpId = CmpID?.CmpID;
                        ...
                        param = new DynamicParameters();
                            ...
                            paramLeaveBal.Add("@p_CmpId", CmpId);
            ...
            public ActionResult IsLeaveExists(string LeaveMasterEmployeeId, string letLeaveMasterLeaveYearId, string LeaveMasterShiftId, string TotalDays, DateTime FormDate, DateTime Todate, string LeaveMasterDayType, int LeaveTypeId)
            {
                try
                {
                    ...
                    var Result = DapperORM.ExecuteReturn("sp_SUD_HRSpace_Leave_Master", param);
            ...
            public ActionResult SaveUpdate(Models.ESS_TimeOffice.Leave_Master leaveMaster, HttpPostedFileBase LeaveFilePath)
            {
                try
                {
                    ...
                    DapperORM.ExecuteReturn("sp_SUD_HRSpace_Leave_Master", param);
                    ...
                    return RedirectToAction("ESS_HRSpace_Leave", "ESS_HRSpace_Leave");
                }
                catch (Exception ex)
                {
                    Session["GetErrorMessage"] = ex.Message;
            ...
            #region this is UnUsed Code ESS_HRSpace_LeaveList 
            [HttpGet]
            public ActionResult ESS_HRSpace_LeaveList()
            {
                try
                {
                    if (Session["EmployeeId"] == null)
            ...
            public ActionResult LeaveDelete(string LeaveMasterId_Encrypted)
            {
                try
                {
                    ...
                    TempData["Message"] = param.Get<string>("@p_msg");
                    TempData["Icon"] = param.Get<string>("@p_Icon");
                    return RedirectToAction("GetList", "ESS_HRSpace_Leave");
                }
                catch (Exception ex)
                {
                    Session["GetErrorMessage"] = ex.Message;
