...
{
    public class ESS_TimeOffice_LeaveController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: ESS/ESS_TimeOffice_Leave
        #region ESS_TimeOffice_Leave Main View 
        [HttpGet]
        public ActionResult ESS_TimeOffice_Leave()
        {
            try
            {
                ...
                param = new DynamicParameters();
                param.Add("@query", "SELECT LeaveTypeId,LeaveTypeShortName FROM Leave_Type,Leave_Setting where Leave_Type.Deactivate = 0 and Leave_Setting.Deactivate=0 and Leave_Type.LeaveTypeId=Leave_Setting.LeaveSettingLeaveTypeId and Leave_Setting.IsActivate=1 and Leave_Setting.LeaveSettingLeaveGroupId=(Select  EM_Atten_LeaveGroupID from Mas_Employee_Attendance where Deactivate=0 and AttendanceEmployeeId=" + Session["EmployeeId"] + " )");
        ...
        public JsonResult GetData(string fromDate, string toDate)
        {
            ...
            ConflictEmployee.Add("@query", $@"Select EmployeeName,Replace(convert(nvarchar(12),FromDate,106),' ','/') as FromDate,Replace(convert(nvarchar(12),ToDate,106),' ','/') as  ToDate 
                                               , TotalDays from Mas_Employee Inner join Leave_Master on Leave_Master.LeaveMasterEmployeeId=Mas_Employee.EmployeeId
                                                 where Mas_Employee.Employeeid in (Select ReportingEmployeeID from MAs_employee_Reporting where MAs_employee_Reporting.Deactivate=0 and  ReportingManager1=" + Session["EmployeeId"] + ") and Mas_Employee.Deactivate=0 and Leave_Master.Deactivate=0  and Mas_Employee.EmployeeId<>" + Session["EmployeeId"] + "  And Leave_Master.FromDate >= '" + fromDate + "' And Leave_Master.FromDate <= '" + toDate + "' ");
        ...
        [HttpGet]
        public ActionResult IsLeaveLog(string letLeaveMasterLeaveYearId, string LeaveMasterShiftId, string TotalDays, DateTime FormDate, DateTime Todate, string LeaveMasterDayType, int LeaveTypeId)
        {
            try
            {
        ...
        public ActionResult SaveUpdate(Models.ESS_TimeOffice.Leave_Master leaveMaster, HttpPostedFileBase LeaveFilePath)
        {
            try
            {
                ...
                // Fetch data
                var Sendmessage = DapperORM.DynamicQueryList("SELECT * FROM tbl_NotificationMessage WHERE Origin='Leave'").FirstOrDefault();
                ...
                return RedirectToAction("ESS_TimeOffice_Leave", "ESS_TimeOffice_Leave");
            }
            catch (Exception ex)
            {
                Session["GetErrorMessage"] = ex.Message;
        ...
        #region this is UnUsed Code ESS_TimeOffice_LeaveList 
        [HttpGet]
        public ActionResult ESS_TimeOffice_LeaveList()
        {
            try
            {
                if (Session["EmployeeId"] == null)
        ...
        public ActionResult LeaveDelete(string LeaveMasterId_Encrypted)
        {
            try
            {
                ...
                TempData["Message"] = param.Get<string>("@p_msg");
                TempData["Icon"] = param.Get<string>("@p_Icon");
                return RedirectToAction("GetList", "ESS_TimeOffice_Leave");
            }
            catch (Exception ex)
            {
                Session["GetErrorMessage"] = ex.Message;
        ...
        [HttpGet]
        public ActionResult GetEmployeeLogAndShift(DateTime LogFromDate, DateTime LogToDate)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                var GetEmployeeCardNo = DapperORM.DynamicQuerySingle(@"Select EmployeeCardNo from Mas_Employee where EmployeeId=" + Session["EmployeeId"] + " and Deactivate=0").FirstOrDefault();
