...
{
    public class ESS_TimeOffice_WeekOffAdjustNewController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: ESS/ESS_TimeOffice_WeekOffAdjustNew
        #region WeekOffAdjust MAin View
        public ActionResult ESS_TimeOffice_WeekOffAdjustNew(Atten_WeekOffAdjust OBJAtten_WeekOffAdjust)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 832;
                ...
                //paramUnit.Add("@query", "select UnitId as Id,UnitName as Name from Mas_Unit where Deactivate=0 and CmpId=" + GetDefaultCmpId + "");
                //var List_SubUnit = DapperORM.ReturnList<AllDropDownBind>("sp_QueryExcution", paramUnit).ToList();
                //ViewBag.GetUnitList = List_SubUnit;
                ...
                if (OBJAtten_WeekOffAdjust.CmpId != null)
                {
                    ...
                    {
                        // Query = " and MAS_EMPLOYEE.EmployeeBranchId=" + OBJAtten_WeekOffAdjust.WOBranchId + "";
                    ...
                    //else
                    //{
                    //    Query = " and MAs_employee.employeeBranchId in (select mas_branch.BranchID as Id from UserBranchMapping,mas_branch where  UserBranchMapping.employeeid = " + Session["EmployeeId"] + " and mas_branch.BranchId = UserBranchMapping.branchid and mas_branch.Deactivate = 0 and mas_branch.CmpId = " + OBJAtten_WeekOffAdjust.CmpId + " and UserBranchMapping.IsActive = 1)";
                    ...
                    var GetFromDate = OBJAtten_WeekOffAdjust.WOAdjustDate.ToString("yyyy-MM-dd");
                    ...
                    {
                        Query = Query + " and MAS_EMPLOYEE.EmployeeBranchId=" + OBJAtten_WeekOffAdjust.WOBranchId + " and MAS_EMPLOYEE.EMPLOYEEID NOT IN (SELECT WOAdjustEmployeeId FROM Atten_WeekOffAdjust where Atten_WeekOffAdjust.WOBranchId = " + OBJAtten_WeekOffAdjust.WOBranchId + " and  Atten_WeekOffAdjust.deactivate = 0 and WOFromDate = '" + GetFromDate + "' Union SELECT WOAdjustEmployeeId FROM Atten_WeekOffAdjust where Atten_WeekOffAdjust.WOBranchId = " + OBJAtten_WeekOffAdjust.WOBranchId + " and Atten_WeekOffAdjust.deactivate = 0 and WOAdjustDate = '" + GetAdjustDate + "' ) and  ( Mas_Employee_Attendance.EM_Atten_WOFF1 = FORMAT(convert(datetime, '" + GetFromDate + "'), 'dddd')  OR Mas_Employee_Attendance.EM_Atten_WOFF2 = FORMAT(convert(datetime, '" + GetFromDate + "'), 'dddd')  )  ";
                        // = Query + " and MAS_EMPLOYEE.EMPLOYEEID NOT IN (SELECT WOAdjustEmployeeId FROM Atten_WeekOffAdjust where Atten_WeekOffAdjust.deactivate=0 and (WOFromDate='" + GetFromDate + "'  or WOAdjustDate='" + GetAdjustDate + "')) and Mas_Employee_Attendance.EM_Atten_WOFF1=FORMAT(convert(datetime,'" + GetFromDate + "'), 'dddd')";
                    }
                    else
                    {
                        Query = Query + " and MAS_EMPLOYEE.EMPLOYEEID NOT IN (SELECT WOAdjustEmployeeId FROM Atten_WeekOffAdjust where Atten_WeekOffAdjust.WOBranchId = " + OBJAtten_WeekOffAdjust.WOBranchId + " and  Atten_WeekOffAdjust.deactivate = 0 and WOFromDate = '" + GetFromDate + "' Union SELECT WOAdjustEmployeeId FROM Atten_WeekOffAdjust where Atten_WeekOffAdjust.WOBranchId = " + OBJAtten_WeekOffAdjust.WOBranchId + " and Atten_WeekOffAdjust.deactivate = 0 and WOAdjustDate = '" + GetAdjustDate + "' ) and ( Mas_Employee_Attendance.EM_Atten_WOFF1 = FORMAT(convert(datetime, '" + GetFromDate + "'), 'dddd')  OR Mas_Employee_Attendance.EM_Atten_WOFF2 = FORMAT(convert(datetime, '" + GetFromDate + "'), 'dddd')  )   and MAS_EMPLOYEE.EMPLOYEEID=" + OBJAtten_WeekOffAdjust.WOAdjustEmployeeId + "";
                    }
        ...
        #region IsValidation
        [HttpPost]
        public ActionResult IsWeekOfAdjustmentExists(DateTime WOAdjustDate, List<WeekOffAdjust> Record, DateTime WOFromDate, int? WOBranchId,string WOAdjustType)
        {
            try
            {
                ...
                for (var i = 0; i < Record.Count; i++)
                {
                    ...
                    param.Add("@p_WOAdjustEmployeeId", Record[i].WOAdjustEmployeeId);
                    param.Add("@p_WOAdjustDate", WOAdjustDate.ToString("yyyy-MM-dd"));
                    param.Add("@p_WOAdjustType", WOAdjustType);
                    param.Add("@p_MachineName", Dns.GetHostName().ToString());
                    param.Add("@p_CreatedUpdateBy", Session["EmployeeName"]);
                    param.Add("@p_msg", dbType: DbType.String, direction: ParameterDirection.Output, size: 500);
                    param.Add("@p_Icon", dbType: DbType.String, direction: ParameterDirection.Output, size: 500);
                    var Result = DapperORM.ExecuteReturn("sp_SUD_Atten_WeekOffAdjust", param);
        ...
        #region SaveUpdate
        [HttpPost]
        public ActionResult SaveUpdate(List<WeekOffAdjust> Record, int? CmpID, int? WODepartmentID, DateTime WOFromDate, DateTime WOAdjustDate,string WOAdjustType)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                if (Record != null)
                {
                    for (var i = 0; i < Record.Count; i++)
                    {
                        ...
                        param.Add("@p_CmpID", CmpID);
                        param.Add("@p_WODepartmentID", WODepartmentID);
                        param.Add("@p_WOAdjustEmployeeId", Record[i].WOAdjustEmployeeId);
                        param.Add("@p_WOFromDate", WOFromDate);
                        param.Add("@p_WOAdjustDate", WOAdjustDate);
                        param.Add("@p_WOAdjustType", WOAdjustType);
                        param.Add("@p_MachineName", Dns.GetHostName().ToString());
                        param.Add("@p_CreatedUpdateBy", Session["EmployeeName"]);
                        param.Add("@p_msg", dbType: DbType.String, direction: ParameterDirection.Output, size: 500);//OutPut Parameter
                        param.Add("@p_Icon", dbType: DbType.String, direction: ParameterDirection.Output, size: 500);//OutPut Parameter
                        var data = DapperORM.ExecuteReturn("sp_SUD_Atten_WeekOffAdjust", param);
                ...
                {
                    return RedirectToAction("ESS_TimeOffice_WeekOffAdjustNew", "ESS_TimeOffice_WeekOffAdjustNew");
                }
            }
            catch (Exception ex)
            {
        ...
        #region GetList
        public ActionResult GetList(WeekOffAdjust_List WeekOffAdjustList, bool chkAll = false)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                if (WeekOffAdjustList.CmpId > 0)
                {
                    if ((WeekOffAdjustList.WOFromDate != null) && (WeekOffAdjustList.WOAdjustDate != null))
                    {
                        DynamicParameters param1 = new DynamicParameters();
                        ...
                        DynamicParameters paramList = new DynamicParameters();
                        paramList.Add("@p_WeekOffAdjustId_Encrypted", "List");
                        paramList.Add("@p_WOFromDate", WeekOffAdjustList.WOAdjustDate);
                        paramList.Add("@p_BranchId", WeekOffAdjustList.WOBranchId);
                        paramList.Add("@p_WOAdjustDate", WeekOffAdjustList.WOAdjustDate);
                        var data = DapperORM.DynamicList("sp_List_Atten_WeekOffAdjust", paramList);
                        ViewBag.GetWeekOffAdjustList = data;
                        TempData["chkAll"] = chkAll ? "1" : "0";
        ...
        public ActionResult Delete(List<WeekOffAdjust_List> ObjRecordList)
        {
            try
            {
                ...
                for (var i = 0; i < ObjRecordList.Count; i++)
                {
                    ...
                    param.Add("@p_WOAdjustEmployeeId", ObjRecordList[i].EmployeeID);
                    param.Add("@p_WOBranchId", ObjRecordList[i].WOBranchId);
                    param.Add("@p_WeekOffAdjustId_Encrypted", ObjRecordList[i].WeekOffAdjustId_Encrypted);
                    param.Add("@p_WOAdjustDate", ObjRecordList[i].WOAdjustDate);
                    param.Add("@p_msg", dbType: DbType.String, direction: ParameterDirection.Output, size: 500);
                    param.Add("@p_Icon", dbType: DbType.String, direction: ParameterDirection.Output, size: 500);
                    var Result = DapperORM.ExecuteReturn("sp_SUD_Atten_WeekOffAdjust", param);
                    TempData["Message"] = param.Get<string>("@p_msg");
