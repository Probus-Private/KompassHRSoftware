...
{
    public class ESS_TMS_TaskStatusReportController : Controller
    {
        ...
        public ActionResult ESS_TMS_TaskStatusReport(DailyAttendanceReportFilter OBJTMSReport)
        {
            try
            {
                ...
                //var GetTMSProject = results[0].Select(x => new AllDropDownBind { Id = (double)x.Id, Name = (string)x.Name }).ToList();

                //ViewBag.TMSProject = GetTMSProject;
                //var ProjectId = GetTMSProject[0].Id;

                //var result1 = DapperORM.DynamicQueryMultiple(@"Select distinct TeamEmployeeId as Id,(Mas_Employee.EmployeeName + ' - ' + CAST(Mas_Employee.EmployeeNo AS VARCHAR)) AS Name from TMS_TeamAssign,Mas_Employee where Mas_Employee.EmployeeId=TMS_TeamAssign.TeamEmployeeId and Mas_Employee.Deactivate=0 and mas_employee.employeeleft = 0 and TMS_TeamAssign.deactivate = 0 order by Name");
                //ViewBag.GetEmployeeName = result1[0].Select(x => new AllDropDownBind { Id = (double)x.Id, Name = (string)x.Name }).ToList();
                ...
                //    {
                //        Query = " and TMS_TaskAssign.ProjectID=" + OBJTMSReport.ProjectID;
                //        if (OBJTMSReport.ModuleId != 0)
                //        {
                //            Query = Query + " and TMS_TaskAssign.ModuleId=" + OBJTMSReport.ModuleId + "";
                //        }
                //        if (OBJTMSReport.AssignToEmployeeID != 0)
                //        {
                //            Query = Query + " and TMS_TaskAssign.AssignToEmployeeID=" + OBJTMSReport.AssignToEmployeeID + "";

                //        }
                //    }

                //    DynamicParameters param1 = new DynamicParameters();
                //    param1.Add("@p_FromDate", OBJTMSReport.FromDate);
                //    param1.Add("@p_ToDate", OBJTMSReport.ToDate);
                //    param1.Add("@P_Qry", Query);
                //    //    param1.Add("@p_EmployeeId", OBJTMSReport.AssignToEmployeeID);
                //    var data = DapperORM.ExecuteSP<dynamic>("sp_GetTaskStatusReports", param1).ToList();
        ...
        public ActionResult DownloadExcelFile(int? AssignToEmployeeID, DateTime fromDate, DateTime ToDate, int ClientID, int ProjectID, int ModuleID)
        {
            try
            {
                ...
                data = DapperORM.ExecuteSP<dynamic>("sp_Rpt_TMS_GetTaskStatusReports", paramList).ToList();
                if (data.Count == 0)
                {
