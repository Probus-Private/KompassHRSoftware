...
{
    public class ESS_TMS_OverdueTasksReportController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);

        // GET: ESS/ESS_TMS_TMSReports
        public ActionResult ESS_TMS_OverdueTasksReport(DailyAttendanceReportFilter OBJTMSReport)
        {
            try
            {
        ...
        [HttpGet]
        public ActionResult GetAssigntoEmployee(int? ProjectId)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                if (ProjectId > 0)
                {
                    query = "TMS_TeamAssign.ProjectID = " + ProjectId;
                }

                param1.Add("@P_Qry", query);

                var assigntoemployee = DapperORM.ReturnList<AllDropDownBind>("sp_TMS_GetAssigntoEmployee", param1).ToList();

                return Json(new { assigntoemployee }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                Session["GetErrorMessage"] = ex.Message;
        ...
        public ActionResult DownloadExcelFile(int? ClientId, int? ProjectId, int? ModuleId, int? EmployeeId, DateTime FromDate, DateTime ToDate)
        {
            try
            {
                ...
                param.Add("@p_FromDate", FromDate);
                param.Add("@p_ToDate", ToDate);
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_TMS_GetOverdueTasksReports", param).ToList();
                if (GetData.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Overdue Tasks Report - (" + FromDate.ToString("dd/MMM/yyyy") + " to " + ToDate.ToString("dd/MMM/yyyy") + ")";
