...
{
    public class App_Leave_CoffController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: App/App_Leave_Coff

        #region Coff
        public ActionResult App_Leave_Coff()
        {
            try
            {
                ...
                //}
                var GetDocNo = DapperORM.DynamicQuerySingle("Select Isnull(Max(DocNo),0)+1 As DocNo from Atten_CoffRequest");
        ...
        public ActionResult App_CoffDetails(int? CoffRequestID)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                //    return RedirectToAction("Dashboard", "Dashboard", new { area = "" });
                //}
                param.Add("@P_Qry", "and Atten_CoffRequest.CoffRequesterEmployeeID=" + Session["EmployeeId"] + " And CoffRequestID=" + CoffRequestID + " ");
                // param.Add("@P_Qry", "and Atten_CoffRequest.CoffRequesterEmployeeID=" + 1001 + " And CoffRequestID=" + CoffRequestID + " ");
                var CoffRequest = DapperORM.ReturnList<dynamic>("sp_List_Atten_CoffRequest", param).ToList();
                ViewBag.CoffRequest = CoffRequest;
        ...
        public ActionResult App_SaveUpdate(Atten_CoffRequest CoffRequest)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                return RedirectToAction("App_Leave_Coff", "App_Leave_Coff");
            }
            catch (Exception ex)
            {
                Session["GetErrorMessage"] = ex.Message;
        ...
        #region CoffRequest Cancel Request
        [HttpGet]
        public ActionResult App_CancelRequest(int? GetCoffRequestID, int ddlManagerId, string Remark, string Origin)
        {
            try
            {
                if (Session["EmployeeId"] == null)
        ...
        #region Coff Delete Request
        [HttpGet]
        public ActionResult App_Delete(string CoffRequestID_Encrypted)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                TempData["Message"] = param.Get<string>("@p_msg");
                TempData["Icon"] = param.Get<string>("@p_Icon");
                return RedirectToAction("App_GetList", "App_Leave_Coff");
            }
            catch (Exception ex)
            {
                Session["GetErrorMessage"] = ex.Message;
