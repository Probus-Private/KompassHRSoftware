...
{
    public class App_ClaimReimbusement_TravelClaimController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: App/App_ClaimReimbusement_TravelClaim
        public ActionResult App_ClaimReimbusement_TravelClaim()
        {
            try
            {
                ...
                //var TravelClaimId = DapperORM.DynamicList("Select Isnull(Max(VoucherNo),0)+1 As TravelClaimId from Claim_Travel where Deactivate=0");
                //TempData["TravelClaimId"] = TravelClaimId.TravelClaimId;

              
                var VoucherNo = DapperORM.DynamicQuerySingle("Select Isnull(Max(VoucherNo),0)+1 As VoucherNo from Claim_Travel");
                ViewBag.VoucherNo = VoucherNo?.VoucherNo;
                ...
                //var IsValid = sqlcon.Query(@"select FromDay from Claim_GeneralSetting where Claim_GeneralSetting.Deactivate=0 and  (day(getdate()) between FromDay  and ToDay ) and CmpId = (select CmpID from Mas_Employee where Mas_Employee.Deactivate=0 and   EmployeeId = " + @Session["EmployeeId"] + " )");
                //TempData["IsValid"] = IsValid.Count();
        ...
        [HttpGet]
        public JsonResult GetTravelRate(string TravelType)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                {
                    return Json(new { error = "SessionExpired" }, JsonRequestBehavior.AllowGet);
                }
                int screenId = Request.QueryString["ScreenId"] != null? Convert.ToInt32(Request.QueryString["ScreenId"]) : 173;
                ...
                var getRate = DapperORM.DynamicQuerySingle( "Select TravelRateAmount from Claim_TravelRate Where VehicalType='" + TravelType + "' and CmpId= " + Session["CompanyId"] + " and TravelRateBranchId=" + Session["BranchId"] + " and Deactivate=0");
        ...
        #region SaveUpdate Travel Cliam
        [HttpPost]
...

        {
            try
            {
                ...
                return RedirectToAction("App_ClaimReimbusement_TravelClaim", "App_ClaimReimbusement_TravelClaim", new { Area = "App" });
            }
            catch (Exception ex)
            {
                Session["GetErrorMessage"] = ex.Message;
        ...
        #region GetList
        public ActionResult App_GetList(string TravelClaimId_Encrypted)
        {
            try
            {
                if (Session["EmployeeId"] == null)
        ...
        //       // System.IO.File.ReadAllBytes(DownloadAttachment);
        ...
        public ActionResult EmployeeChange()
        {
            try
            {
                ...
                param.Add("@query", "select TravelRateId,VehicalType from Claim_TravelRate Rate Right join Mas_Branch Branch On Branch.BranchId = Rate.TravelRateBranchId where Rate.CmpId=" + Session["CompanyId"] + " and  Branch.BranchId=" + Session["BranchId"] + "");
        ...
        [HttpGet]
        public ActionResult App_TravelDetails(int? TravelClaimId)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                param.Add("@p_TravelClaimId_Encrypted", "List");
                param.Add("@P_Qry", "and Claim_Travel.TravelClaimEmployeeId=" + Session["EmployeeId"] + " And TravelClaimId=" + TravelClaimId + " ");
                var TravelClaim = DapperORM.ReturnList<dynamic>("sp_List_Claim_Travel", param).ToList();
                ViewBag.TravelClaim = TravelClaim;
        ...
        public ActionResult TravelClaimDelete(string TravelClaimId_Encrypted)
        {
            try
            {
                ...
                return RedirectToAction("App_GetList", "App_ClaimReimbusement_TravelClaim", new { Area = "App" });
            }
            catch (Exception ex)
            {
                Session["GetErrorMessage"] = ex.Message;
