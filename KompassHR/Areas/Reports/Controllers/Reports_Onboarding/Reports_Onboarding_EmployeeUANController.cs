...
{
    public class Reports_Onboarding_EmployeeUANController : Controller
    {
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        DynamicParameters param = new DynamicParameters();

        // GET: Reports/Reports_Onboarding_EmployeeUAN
        public ActionResult Reports_Onboarding_EmployeeUAN(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 407;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("EmployeeUANReport");
                ...
                if (CmpId != null)
                {
                    ...
                    var GetFromDate = FromDate;
                   
                    if (FromDate != null)
                    {
                        Query = Query + " and month( Atten_InOut.InOutDate )='" + GetFromDate.ToString("MM") + "' and year( Atten_InOut.InOutDate )='" + GetFromDate.ToString("yyyy") +"' ";
                    }
                }
                DynamicParameters paramList = new DynamicParameters();
                paramList.Add("@P_Qry", Query);
                var GetEmployeeUANReport = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Onboarding_EmployeeUAN", paramList).ToList();
                if (GetEmployeeUANReport.Count == 0)
                {
                    byte[] emptyFileContents = new byte[0];
                    return File(emptyFileContents, "application/octet-stream", "FileNotFound.txt");
                }
                DapperORM dprObj = new DapperORM();
                dt = dprObj.ConvertToDataTable(GetEmployeeUANReport);
                ...
                worksheet.Cell(1, 1).Value = "Employee UAN Report - (" + FromDate.ToString("MMM/yyyy") + ")";
