...
{
    public class Reports_Onboarding_DocumentSubmissionStatusReportController : Controller
    {
        // GET: Reports/Rpt_Payroll_BranchUnitWiseSummary
        public ActionResult Reports_Onboarding_DocumentSubmissionStatusReport(MonthWiseFilter MonthWiseFilter)
        {
            try
            {
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("DocumentSubmissionStatusReport");
                ...
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Onboarding_DocumentSubmissionStatus", paramList).ToList();
                if (GetData.Count == 0)
                {
                ...
                int documentColumnStart = 7;

                for (int row = 3; row <= totalRows + 1; row++) // Start from 3 to skip header/title
                {
                    for (int col = documentColumnStart; col <= dt.Columns.Count; col++)
                    {
                        var cell = worksheet.Cell(row, col);
                        var cellValue = cell.GetValue<string>().Trim().ToLower();
                ...
                worksheet.Cell(1, 1).Value = "Document Submission Status Report";
