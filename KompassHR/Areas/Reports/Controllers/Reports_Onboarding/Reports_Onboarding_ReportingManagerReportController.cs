...
{
    public class Reports_Onboarding_ReportingManagerReportController : Controller
    {
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        DynamicParameters param = new DynamicParameters();
        // GET: Reports/Reports_Onboarding_ReportingManagerReport
        #region Reports_Onboarding_ReportingManagerReport
        public ActionResult Reports_Onboarding_ReportingManagerReport(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 384;
                ...
                paramName.Add("@query", "select ModuleID as Id ,ModuleName as Name from Tool_Module where IsApplicable = 1 and Deactivate = 0");
                ViewBag.ModuleName = DapperORM.ReturnList<AllDropDownBind>("sp_QueryExcution", paramName).ToList();
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId,int? ModuleId,int? ContractorId)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                var worksheet = workbook.Worksheets.Add("ReportingManagerReport");
                ...
                if (CmpId != null)
                {
                    ...
                    if (ModuleId != null)
                    {
                        Query = Query + " and Mas_Employee_Reporting.ReportingModuleID=" + ModuleId + "";
                    }
                }
                DynamicParameters paramList = new DynamicParameters();
                paramList.Add("@P_Qry", Query);
                var LeftEmployeeReportList = DapperORM.ReturnList<dynamic>("sp_Rpt_Onboarding_ReportingManager", paramList).ToList();
                ...
                worksheet.Cell(1, 1).Value = "Reporting Manager Report - (" + FromDate.ToString("dd/MMM/yyyy") + ")";
