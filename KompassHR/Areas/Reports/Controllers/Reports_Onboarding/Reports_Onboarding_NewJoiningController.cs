...
{
    public class Reports_Onboarding_NewJoiningController : Controller
    {
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        DynamicParameters param = new DynamicParameters();
        // GET: /Reports/Reports_Onboarding_NewJoining/Reports_Onboarding_NewJoining
        #region Reports_Onboarding_NewJoining
        public ActionResult Reports_Onboarding_NewJoining(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 296;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? ContractorId, int? GradeId, DateTime ToDate)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                var worksheet = workbook.Worksheets.Add("NewJoiningEmployeeReport");
                ...
                if (CmpId != null)
                {
                    ...
                    if (FromDate != null)
                    {
                        Query = Query + " and JoiningDate between '" + GetFromDate + "' and '" + GetToDate + "'";
                    }
                }
                DynamicParameters paramList = new DynamicParameters();
                paramList.Add("@P_Qry", Query);
                var NewJoiningReport = DapperORM.ReturnList<dynamic>("Sp_Rpt_Onboarding_NewJoiningEmployeeList", paramList).ToList();
                if (NewJoiningReport.Count == 0)
                {
                    byte[] emptyFileContents = new byte[0];
                    return File(emptyFileContents, "application/octet-stream", "FileNotFound.txt");
                }
                DapperORM dprObj = new DapperORM();
                dt = dprObj.ConvertToDataTable(NewJoiningReport);
                ...
                worksheet.Cell(1, 1).Value = "New Joining Employee Report - (" + FromDate.ToString("dd/MMM/yyyy") + " - " + ToDate.ToString("dd/MMM/yyyy") + ")";
