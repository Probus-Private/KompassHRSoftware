...
{
    public class Reports_Asset_AssetWiseReportController : Controller
    {
        #region Main View 
        // GET: Reports/Reports_Asset_AssetWiseReport
        public ActionResult Reports_Asset_AssetWiseReport(EmployeeWiseReportFilter EmployeeReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 842;
                ...
                if (EmployeeReport.CmpId != 0 && EmployeeReport.BranchId != null)
                {
                    ...
                    DynamicParameters paramAsset = new DynamicParameters();
                    ...
                    var AssetList = DapperORM.ReturnList<AllDropDownBind>("sp_QueryExcution", paramAsset).ToList();
                    ViewBag.AssetName = AssetList;
                }
              
                else
                {
                    ViewBag.AssetName = new List<AllDropDownBind>();
                }
        ...
        public ActionResult GetBusinessUnit(int CmpId)
        {
            try
            {
                ...
                var AssetName = DapperORM.ReturnList<AllDropDownBind>("sp_QueryExcution", paramAsset).ToList();
               // return Json(data, JsonRequestBehavior.AllowGet);

                return Json(new { Branch = Branch, AssetName = AssetName }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                Session["GetErrorMessage"] = ex.Message;
        ...
        #region GetAssetName
        [HttpGet]
        public ActionResult GetAssetName(int CmpId, int? BranchId)
        {
            try
            {
                if (Session["EmployeeId"] == null)
               ...
               // DynamicParameters param = new DynamicParameters();
                ...
                if (BranchId == 0 || BranchId == null)
                {
                    DynamicParameters paramAsset = new DynamicParameters();
                    paramAsset.Add("@query", "SELECT DISTINCT Mas_Asset.AssetId AS[Id], Mas_Asset.AssetName AS [Name] FROM Mas_Employee INNER JOIN Mas_Employee_Asset MEA ON Mas_Employee.EmployeeId = MEA.AssetEmployeeId INNER JOIN Mas_Asset  ON MEA.AssetName = Mas_Asset.AssetId INNER JOIN Mas_CompanyProfile ON Mas_Employee.CmpId = Mas_CompanyProfile.CompanyId WHERE Mas_Employee.Deactivate = 0 AND Mas_CompanyProfile.CompanyId = '" + CmpId + "'order By Name");
                   // param.Add("@query", "select AssetId as Id,AssetName as Name  from Mas_Asset where Deactivate=0 and CmpID ='" + CmpId + "' order by Name");
                    var AssetName = DapperORM.ReturnList<AllDropDownBind>("sp_QueryExcution", paramAsset).ToList();
                ...
                else
                {
                    ...
                    //paramEmp.Add("@query", "select EmployeeId as Id, Concat(EmployeeName ,' - ', EmployeeNo) as Name  from Mas_Employee where Deactivate=0 and CmpID ='" + CmpId + "' and EmployeeBranchId=" + BranchId + "  order by Name");
                    var AssetName = DapperORM.ReturnList<AllDropDownBind>("sp_QueryExcution", paramAsset).ToList();
                    return Json(AssetName, JsonRequestBehavior.AllowGet);
                }
            }
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, int? AssetId, DateTime? Month = null)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                var worksheet = workbook.Worksheets.Add("Asset Wise Report");
                ...
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Asset_AssetWiseReport", paramList).ToList();
               ...
               // lastRow.Style.Font.FontColor = XLColor.Black;
                ...
                worksheet.Cell(1, 1).Value = Month.HasValue ? $"Asset Wise Report - ({Month.Value:dd/MMM/yyyy})" : "Asset Wise Report";
