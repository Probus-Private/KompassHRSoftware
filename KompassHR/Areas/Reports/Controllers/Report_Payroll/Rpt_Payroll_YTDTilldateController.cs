...
{
    public class Rpt_Payroll_YTDTilldateController : Controller
    {
        // GET: Reports/Rpt_Payroll_YTDTilldate
        #region Main View
        public ActionResult Rpt_Payroll_YTDTilldate()
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 854;
                ...
                DynamicParameters paramfy = new DynamicParameters();
                paramfy.Add("@query", "select Fid as Id , cast(year(FFrom) as nvarchar(4))+'-'+cast(YEAR(FTo) as nvarchar(4)) as Name from Tool_FinancialYear");
                var FinancialYear = DapperORM.ReturnList<AllDropDownBind>("sp_QueryExcution", paramfy).ToList();
                ViewBag.GetFinancialYear = FinancialYear;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId,DateTime FromMonthYear, DateTime ToMonthYear)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("YTDSummaryReport");
                ...
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Payroll_YTDSummary", paramList).ToList();
                if (GetData.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "YTD Summary Report - " + FromMonthYear.ToString("MMM/yyyy") + " to " + ToMonthYear.ToString("MMM/yyyy") +"";
