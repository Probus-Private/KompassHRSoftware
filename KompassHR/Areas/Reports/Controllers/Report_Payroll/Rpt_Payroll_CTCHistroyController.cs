...
{
    public class Rpt_Payroll_CTCHistroyController : Controller
    {
        // GET: Reports/Rpt_Payroll_CTCHistroy
        public ActionResult Rpt_Payroll_CTCHistroy(MonthWiseFilter MonthWiseFilter)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 908;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId,DateTime FromDate,DateTime ToDate)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("CTCHistroyReport");
             ...
             //   paramList.Add("@p_EmployeeId", Session["EmployeeId"]);
                paramList.Add("@CompanyId", CmpId);
                paramList.Add("@BranchId", BranchId);
                paramList.Add("@FromDate", FromDate);
                paramList.Add("@ToDate", ToDate);
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_rpt_Payroll_CTC_Monthlywise_History", paramList).ToList();
                if (GetData.Count == 0)
                {
                ...
                int monthStartCol = 9; // 1-3 fixed columns

                // Range: from 2nd row, 5th column to last row, last column
                var rng = worksheet.Range(3, monthStartCol + 1, lastRow, lastCol);

                string formula = $"=I3<>J3";
                // IMPORTANT: E2 and D2 must match first comparison column
               ...
               // lastRow.Style.Font.FontSize = 10;
                ...
                worksheet.Cell(1, 1).Value = "CTC Histroy Report (" + FromDate.ToString("dd/MMM/yyyy") + " - " + ToDate.ToString("dd/MMM/yyyy") + ")";
