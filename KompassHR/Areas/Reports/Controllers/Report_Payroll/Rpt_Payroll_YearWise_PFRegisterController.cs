...
{
    public class Rpt_Payroll_YearWise_PFRegisterController : Controller
    {
        // GET: Reports/Rpt_Payroll_YearWise_PFRegister
        #region Main View
        public ActionResult Rpt_Payroll_YearWise_PFRegister()
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 902;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromMonthYear, DateTime ToMonthYear)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("YearWisePF");
                ...
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Payroll_YearWisePF", paramList).ToList();
                if (GetData.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Year Wise PF Report - (" + FromMonthYear.ToString("dd/MMM/yyyy") + " to " + ToMonthYear.ToString("dd/MMM/yyyy") + ")";
        ...
        public ActionResult Print(int? CmpId, int? BranchId,DateTime FromMonthYear, DateTime ToMonthYear)
        {
            try
            {
                ...
                html.Append($"<title>Year Wise Payroll PF Register Report - ({FromMonthYear:MMM yyyy} -{ToMonthYear:MMM yyyy})</title>");
                html.Append(@"<style>
            ...
            .right { text-align: right; }
                ...
                html.Append($"<h3 style='text-align:center;'>Year Wise Payroll PF Register Report - ({FromMonthYear:MMM yyyy} -{ToMonthYear:MMM yyyy})</h3>");
