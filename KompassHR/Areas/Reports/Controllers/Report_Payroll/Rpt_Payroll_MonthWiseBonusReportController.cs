...
{
    public class Rpt_Payroll_MonthWiseBonusReportController : Controller
    {
        #region  // GET: Reports/Rpt_Payroll_BranchUnitWiseSummary
        public ActionResult Rpt_Payroll_MonthWiseBonusReport(MonthWiseFilter MonthWiseFilter)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 701;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime Month)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("MonthWiseBonusReport");
                ...
                paramList.Add("@p_process", "MonthWiseBonus");
                paramList.Add("@p_EmployeeId", Session["EmployeeId"]);
                paramList.Add("@p_FromMonthYear", Month);
                paramList.Add("@p_ToMonthYear", Month); // Add this line
                paramList.Add("@p_CompanyId", CmpId);
                paramList.Add("@p_BranchId", BranchId);
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Payroll_BonusReport", paramList).ToList();
                if (GetData.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Month Wise Bonus Report - (" + Month.ToString("dd/MMM/yyyy") + ")";
