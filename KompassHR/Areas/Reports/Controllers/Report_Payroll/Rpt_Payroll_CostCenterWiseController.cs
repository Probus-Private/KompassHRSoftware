...
{
    public class Rpt_Payroll_CostCenterWiseController : Controller
    {
        // GET: Reports/Rpt_Payroll_BranchUnitWiseSummary
        public ActionResult Rpt_Payroll_CostCenterWise(MonthWiseFilter MonthWiseFilter)
        {
            try
            {
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime Month)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("CostCenterWiseReport");
                ...
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Payroll_CostCenterWiseReport", paramList).ToList();
                if (GetData.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Prveious Vs Current Salary Report - (" + Month.ToString("dd/MMM/yyyy") + ")";
