...
{
    public class Rpt_Payroll_PTRegisterController : Controller
    {
        // GET: Reports/Rpt_Payroll_PTRegister
        #region Main View
        public ActionResult Rpt_Payroll_PTRegister()
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 461;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime Month)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("PTRegisterReport");
                ...
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Payroll_PT", paramList).ToList();
                if (GetData.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Payroll PT Register Report - (" + Month.ToString("dd/MMM/yyyy") + ")";
        ...
        public ActionResult Print(int? CmpId, int? BranchId, DateTime Month)
        {
            try
            {
                ...
                var detailData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Payroll_PT", paramList).ToList();
                ...
                paramSummary.Add("@p_MonthYear", Month);
                paramSummary.Add("@p_CompanyId", CmpId);
                paramSummary.Add("@p_BranchId", BranchId);

                var summaryData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Payroll_PTSummary", paramSummary).ToList();
                ...
                html.Append($"<title>Payroll PT Register Report - ({Month:MMM yyyy})</title>");
                html.Append("<style>");
                ...
                html.Append($"<h3 style='text-align:left;margin-bottom:10px;'>Payroll PT Register Report - ({Month:MMM yyyy})</h3>");
