...
{
    public class Rpt_Payroll_BudgetVsActualController : Controller
    {
        // GET: Reports/Rpt_Payroll_BranchUnitWiseSummary
        public ActionResult Rpt_Payroll_BudgetVsActual(MonthWiseFilter MonthWiseFilter)
        {
            try
            {
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime Month)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("BudgetVsActualReport");
                ...
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Payroll_BudgetAndActualReport", paramList).ToList();
                if (GetData.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Budget Vs Actual Report - (" + Month.ToString("dd/MMM/yyyy") + ")";
