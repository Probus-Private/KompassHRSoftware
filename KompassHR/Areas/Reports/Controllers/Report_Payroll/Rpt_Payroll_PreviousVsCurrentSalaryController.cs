...
{
    public class Rpt_Payroll_PreviousVsCurrentSalaryController : Controller
    {
        // GET: Reports/Rpt_Payroll_BranchUnitWiseSummary
        public ActionResult Rpt_Payroll_PreviousVsCurrentSalary(MonthWiseFilter MonthWiseFilter)
        {
            try
            {
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime Month)
        {
            try
            {
                ...
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Payroll_CurrentVsPreviousSalaryReport", paramList).ToList();
                if (GetData.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Salary Hold & Release Report - (" + Month.ToString("dd/MMM/yyyy") + ")";
