...
{
    public class Rpt_Payroll_ProcessSalaryController : Controller
    {
        // GET: Reports/Rpt_Payroll_BranchUnitWiseSummary
        public ActionResult Rpt_Payroll_ProcessSalary(MonthWiseFilter MonthWiseFilter)
        {
            try
            {
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime Month)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("ProcessSalaryReport");
                ...
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Payroll_PayrollSalary", paramList).ToList();
                if (GetData.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Process Salary - (" + Month.ToString("dd/MMM/yyyy") + ")";
