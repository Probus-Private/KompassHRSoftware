...
{
    public class Rpt_Payroll_LWFRegisterController : Controller
    {
        // GET: Reports/Rpt_Payroll_LWFRegister
        #region Main View
        public ActionResult Rpt_Payroll_LWFRegister()
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 462;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime Month)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("LWFReport");
                ...
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Payroll_LWF", paramList).ToList();
                if (GetData.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Payroll LWF Report - (" + Month.ToString("dd/MMM/yyyy") + ")";
        ...
        public ActionResult Print(int? CmpId, int? BranchId, DateTime Month)
        {
            ...
            try
            {
                ...
                html.Append($"<title>Payroll LWF Register Report - ({Month:MMM yyyy})</title>");
                ...
                // Report heading
                html.Append($"<h3 style='text-align:left;margin-bottom:10px;'>Payroll LWF Register Report - ({Month:MMM yyyy})</h3>");
                ...
                var summaryData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Payroll_LWFSummary", paramSummary)?.ToList()
                                  ?? new List<dynamic>();
