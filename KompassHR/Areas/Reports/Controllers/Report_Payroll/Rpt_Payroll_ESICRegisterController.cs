...
{
    public class Rpt_Payroll_ESICRegisterController : Controller
    {
        // GET: Reports/Rpt_Payroll_ESICRegister
        #region Main View
        public ActionResult Rpt_Payroll_ESICRegister()
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 459;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime Month)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("ESICRegisterReport");
                ...
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Payroll_ESIC", paramList).ToList();
                if (GetData.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Payroll ESIC Register Report - (" + Month.ToString("dd/MMM/yyyy") + ")";
        ...
        public ActionResult Print(int? CmpId, int? BranchId, DateTime Month)
        {
            try
            {
                ...
                List<dynamic> GetSummary = new List<dynamic>();
                DynamicParameters MulQuery = new DynamicParameters();
                MulQuery.Add("@p_EmployeeId", Session["EmployeeId"]);
                MulQuery.Add("@p_MonthYear", Month);
                MulQuery.Add("@p_CompanyId", CmpId);
                MulQuery.Add("@p_BranchId", BranchId);
                using (var multi = DapperORM.DynamicMultipleResultList("sp_Rpt_Payroll_ESIC", MulQuery))
                {

                    GetData = multi.Read<dynamic>().ToList();
                    GetSummary = multi.Read<dynamic>().ToList();
                }
                ...
                html.Append($"<title>Payroll ESIC Register Report - ({Month:MMM yyyy})</title>");
                ...
                // Report heading
                html.Append($"<h3 style='text-align:left;margin-bottom:10px;'>Payroll ESIC Register Report - ({Month:MMM yyyy})</h3>");
                ...
                // ================== SUMMARY SECTION ==================
                // ================== SUMMARY SECTION ==================
                if (GetSummary.Count > 0)
                {
                    var firstSummaryRow = (IDictionary<string, object>)GetSummary.First();

                    html.Append("<br/><h3 style='text-align:left;margin-bottom:10px;'>Summary</h3>");
                    html.Append("<table border='1' style='border-collapse:collapse;width:60%;font-size:12px;margin:auto;'>");

                    // Table header
                    html.Append("<thead><tr style='background-color:#cddcac;font-weight:bold;'>");
                    foreach (var col in firstSummaryRow.Keys)
                    {
                        html.Append($"<th style='padding:5px;'>{col}</th>");
                    }
                    html.Append("</tr></thead>");
                    ...
                    // Table rows
                    html.Append("<tbody>");
                    foreach (var row in GetSummary)
                    {
                        html.Append("<tr>");
                        foreach (var kv in (IDictionary<string, object>)row)
                        {
                            html.Append($"<td style='padding:5px;text-align:right;'>{kv.Value}</td>");
                        }
                        html.Append("</tr>");
                    }
                    html.Append("</tbody></table>");
