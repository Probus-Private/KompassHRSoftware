...
{
    public class Rpt_Payroll_EarningVsDedutionController : Controller
    {
        // GET: Reports/Rpt_Payroll_BranchUnitWiseSummary
        public ActionResult Rpt_Payroll_EarningVsDedution(MonthWiseFilter MonthWiseFilter)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 711;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromMonthYear, DateTime ToMonthYear,int Percentage)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                var worksheet = workbook.Worksheets.Add("EarningVsDeductionReport");
                ...
                paramList.Add("@p_Percentage", Percentage);
                
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Payroll_EarningVsDeductionReport", paramList).ToList();
                if (GetData.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Earning Vs Deduction Report - (" + FromMonthYear.ToString("dd/MMM/yyyy") + " to " + ToMonthYear.ToString("dd/MMM/yyyy") + ")";
