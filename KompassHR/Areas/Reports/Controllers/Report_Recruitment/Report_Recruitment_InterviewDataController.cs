...
{
    public class Report_Recruitment_InterviewDataController : Controller
    {
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        DynamicParameters param = new DynamicParameters();
        // GET: Reports/Report_Recruitment_InterviewData
        #region Report_Recruitment_InterviewData
        public ActionResult Report_Recruitment_InterviewData(DailyAttendanceReportFilter MasReport)
        {
            try
            {
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, int? DepartmentId, int? DesignationId, DateTime FromDate,DateTime ToDate)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("InterviewDataReport");
                ...
                if (CmpId != null)
                {
                    ...
                    if (FromDate != null)
                    {
                        Query = Query + " and CONVERT(date, Recruitment_Interview.InterviewDate) BETWEEN  '" + GetFromdate + "' AND '" + GetTodate + "'";
                    }
                ...
                data = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Recruitment_InterviewData", paramList).ToList();
                if (data.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Interview Data Report - (" + FromDate.ToString("dd/MMM/yyyy") + " - " + ToDate.ToString("dd/MMM/yyyy") + ")";
