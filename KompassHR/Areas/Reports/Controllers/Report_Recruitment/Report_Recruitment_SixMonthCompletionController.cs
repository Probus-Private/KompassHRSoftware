...
{
    public class Report_Recruitment_SixMonthCompletionController : Controller
    {
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        DynamicParameters param = new DynamicParameters();
        GetMenuList ClsGetMenuList = new GetMenuList();
        // GET: Reports/Report_Recruitment_SixMonthCompletion
        #region Report_Recruitment_SixMonthCompletion
        public ActionResult Report_Recruitment_SixMonthCompletion()
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 615;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? DepartmentId, int? DesignationId, int? AgencyId, DateTime ToDate)
        {
            try
            {
                ...
                if (CmpId != null)
                {
                    ...
                    //if (FromDate != null)
                    //{
                    //    Query = Query + " and CONVERT(date, Atten_InOut.InOutDate) BETWEEN  '" + GetFromDate + "' AND '" + GetToDate + "'  and Recruitment_Resume.Mas_Employee_EmployeeId is not null";
                    //}
                }
                ...
                var Recruitment_AgencyDetails = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Recruitment_SixMonthCompletion", paramList).ToList();
