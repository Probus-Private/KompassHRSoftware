...
{
    public class Report_Recruitment_AgencySelectedController : Controller
    {
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        DynamicParameters param = new DynamicParameters();
        GetMenuList ClsGetMenuList = new GetMenuList();
        // GET: Reports/Report_Recruitment_AgencySelected
        #region Report_Recruitment_AgencySelected
        public ActionResult Report_Recruitment_AgencySelected()
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 614;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? DepartmentId, int? DesignationId, int? AgencyId, DateTime ToDate)
        {
            try
            {
                ...
                if (CmpId != null)
                {
                    ...
                    if (FromDate != null)
                    {
                        Query = Query + " and CONVERT(date, Recruitment_Resume.DocDate) BETWEEN  '" + GetFromDate + "' AND '" + GetToDate + "'  and Recruitment_Resume.Mas_Employee_EmployeeId is not null";
                    }
                }
                DynamicParameters paramList = new DynamicParameters();
                paramList.Add("@P_Qry", Query);
                var Recruitment_AgencyDetails = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Recruitment_AgencySelectedDetails", paramList).ToList();
