...
{
    public class Rpt_ExitManagement_ResignationReportController : Controller
    {
        // GET: Reports/Rpt_ExitManagement_ResignationReport
        public ActionResult Rpt_ExitManagement_ResignationReport(MonthWiseFilter MonthWiseFilter)
        {
            try
            {
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromMonthYear, DateTime ToMonthYear)
        {
            try
            {
                ...
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_FNF_ResignationReport", paramList).ToList();
                if (GetData.Count == 0)
                {
               ...
               // lastRow.Style.Font.Bold = true;
                ...
                worksheet.Cell(1, 1).Value = "Resignation Report - (" + FromMonthYear.ToString("dd/MMM/yyyy") + " to " + ToMonthYear.ToString("dd/MMM/yyyy") + ")";
