...
{
    public class Rpt_ExitManagement_YearlyDeptwiseResignationSummaryController : Controller
    {
        // GET: Reports/Rpt_ExitManagement_YearlyDeptwiseResignationSummary
        public ActionResult Rpt_ExitManagement_YearlyDeptwiseResignationSummary(MonthWiseFilter MonthWiseFilter)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 868;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromMonthYear, DateTime ToMonthYear)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("YearlyDeptwiseResignation");
                ...
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_FNF_Yearly_DepartmentwiseResignation_Summary", paramList).ToList();
                if (GetData.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Yearly Department wise Resignation Summary Report - (" + FromMonthYear.ToString("dd/MMM/yyyy") + " to " + ToMonthYear.ToString("dd/MMM/yyyy") + ")";
