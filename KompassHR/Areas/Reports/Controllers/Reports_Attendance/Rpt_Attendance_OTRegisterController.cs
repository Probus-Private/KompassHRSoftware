...
{
    public class Rpt_Attendance_OTRegisterController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: Reports/Rpt_Attendance_OTRegister
        #region OTRegister
        public ActionResult Rpt_Attendance_OTRegister(DailyAttendanceReportFilter Obj)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 381;
        ...
        public ActionResult DownloadExcelFile(int? BranchId, DateTime FromDate, int CmpId, int? ContractorId)
        {
            try
            {
                ...
                foreach (var branch in selectedBranches)
                {
                    ...
                    data = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Atten_OTRegister", param).ToList();
                    ...
                    // Set the header row
                    worksheet.Cell(1, 1).Value = "Attendance OT Register Report  - (" + FromDate.ToString("MMM/yyyy") + ") - " + branchName;
