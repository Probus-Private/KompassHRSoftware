...
{
    public class Reports_Attendance_LineWiseMonthlyHeadCountController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: Reports/Reports_Attendance_LineWiseMonthlyHeadCount
        #region Reports_Attendance_LineWiseMonthlyHeadCount
        public ActionResult Reports_Attendance_LineWiseMonthlyHeadCount(HeadCountSummary HeadCount)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 346;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, DateTime ToDate)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("LineWiseMonthlyHeadCount");
                ...
                if (CmpId != null)
                {
                    ...
                    {
                        Query = Query + "and mas_employee.JoiningDate<='" + GetFromDate + "' and (mas_employee.LeavingDate>='" + GetToDate + "' or LeavingDate is null) ";
                    }
                }
                param.Add("@p_qry", Query);
                data = DapperORM.ReturnList<dynamic>("sp_Rpt_Atten_LineWiseMonthlyHeadCount", param).ToList();
                if (data.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Line Wise Monthly Head Count Report - (" + FromDate.ToString("dd/MMM/yyyy") + " To " + ToDate.ToString("dd/MMM/yyyy") + ")";
