...
{
    public class Report_YearlyAttendance_EmployeeDetailsController : Controller
    {
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        DynamicParameters param = new DynamicParameters();
        // GET: Reports/Report_YearlyAttendance_EmployeeDetails
        #region Report_YearlyAttendance_EmployeeDetails
        public ActionResult Report_YearlyAttendance_EmployeeDetails(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 480;
        ...
        public ActionResult DownloadExcelFile(int BranchId, DateTime FromDate, int? ContractorId, DateTime ToDate)
        {
            try
            {
                ...
                {
                    Query += " and Atten_InOut.InOutBranchId=" + BranchId +
                             " and Atten_InOut.InOut_EmpContractorId=" + ContractorId +
                             " and Atten_InOut.InOutMonthYear BETWEEN '" + FromDate.ToString("yyyy-MM-dd") +
                ...
                data = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Atten_YearlyEmployeeDetails", paramList).ToList();
                if (data.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Yearly Attendance Details Report - (" + FromDate.ToString("MMM/yyyy") + ") - (" + ToDate.ToString("MMM/yyyy") + ")";
