...
{
    public class Rpt_Attendance_PersonalGatepassController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: Reports/Rpt_Attendance_PersonalGatepass
        #region Rpt_Attendance_PersonalGatepass
        public ActionResult Rpt_Attendance_PersonalGatepass(DailyAttendanceReportFilter Daily_AttendanceReportFilter)
        {
            try
            {
                ...
                return View(Daily_AttendanceReportFilter);
            }
            catch (Exception ex)
            {
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate)
        {
            ...
            var worksheet = workbook.Worksheets.Add("PersonalGatepassReport");
            ...
            if (CmpId != null)
            {
                ...
                {
                    Query = Query + " and Atten_PersonalGatepass.Status='Approved' and  CONVERT(date, Atten_PersonalGatepass.PersonalGatepassDate, 106) BETWEEN  '" + FromDate.ToString("yyyy-MM-dd") + "' AND '" + ToDate.ToString("yyyy-MM-dd") + "'";
                }

            }
            DynamicParameters paramList = new DynamicParameters();
            paramList.Add("@P_Qry", Query);
            var PersonalGatePasstList = DapperORM.ReturnList<dynamic>("sp_List_Atten_PersonalGatepass", paramList).ToList();
            if (PersonalGatePasstList.Count == 0)
            {
                byte[] emptyFileContents = new byte[0];
                return File(emptyFileContents, "application/octet-stream", "FileNotFound.txt");
            }
            DapperORM dprObj = new DapperORM();
            dt = dprObj.ConvertToDataTable(PersonalGatePasstList);
            ...
            worksheet.Cell(1, 1).Value = "Personal Gatepass Report - (" + FromDate.ToString("dd/MMM/yyyy") + " - " + ToDate.ToString("dd/MMM/yyyy") + ")";
            ...
            // Save the workbook
            string SavefilePath = Server.MapPath("~/assets/TempReport/Atten_PersonalGatepass.xlsx");
            workbook.SaveAs(SavefilePath);

            // Path to the file you want to download
            string filePath = Server.MapPath("~/assets/TempReport/Atten_PersonalGatepass.xlsx");
            ...
            string fileName = "PersonalGatepass - (" + FromDate.ToString("MMM/yyyy") + ").xlsx";
