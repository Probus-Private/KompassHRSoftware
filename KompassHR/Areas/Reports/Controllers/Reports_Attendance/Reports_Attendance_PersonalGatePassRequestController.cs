...
{
    public class Reports_Attendance_PersonalGatePassRequestController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: Reports/Reports_Attendance_PersonalGatePassRequest

        #region PersonalGatePassRequest
        public ActionResult Reports_Attendance_PersonalGatePassRequest(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 368;
        ...
        public ActionResult GetUnit(int? CmpId, int? UnitBranchId)
        {
            try
            {
                ...
                else if (UnitBranchId == null)
                {
                    DynamicParameters param = new DynamicParameters();
                    param.Add("@query", "select EmployeeId as Id, Concat(EmployeeName ,' - ', EmployeeNo) as Name  from Mas_Employee where Deactivate=0 and CmpID= " + CmpId + " and Mas_Employee.EmployeeLeft=0");
                    var EmployeeName = DapperORM.ReturnList<AllDropDownBind>("sp_QueryExcution", param).ToList();
                    var List_LineMaster = "";
                    var List_SubUnit = "";
                    return Json(new { EmployeeName = EmployeeName, List_LineMaster = List_LineMaster, List_SubUnit = List_SubUnit }, JsonRequestBehavior.AllowGet);
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("PersonalGatePassRequestReport");
                ...
                if (CmpId != null)
                {
                    Query = " and Atten_PersonalGatepass.CmpId=" + CmpId + "";
                    ...
                    {
                        Query = Query + " and mas_Employee.EmployeeBranchId in (select mas_branch.BranchID as Id from UserBranchMapping,mas_branch where  UserBranchMapping.employeeid = " + Session["EmployeeId"] + " and mas_branch.BranchId = UserBranchMapping.branchid and mas_branch.Deactivate = 0 and mas_branch.CmpId = " + CmpId + " and UserBranchMapping.IsActive = 1)";
                    }
                    ...
                    {
                        Query = Query + " and CONVERT(date, Atten_PersonalGatepass.PersonalGatepassDate) BETWEEN  '" + GetFromDate + "' AND '" + GetToDate + "' ";
                    }
                }
                DynamicParameters paramList = new DynamicParameters();
                paramList.Add("@P_Qry", Query);
                var GetPersonalGatePassRequest = DapperORM.ReturnList<dynamic>("sp_Rpt_Atten_ESS_PersonalGatepass", paramList).ToList();
                if (GetPersonalGatePassRequest.Count == 0)
                {
                    byte[] emptyFileContents = new byte[0];
                    return File(emptyFileContents, "application/octet-stream", "FileNotFound.txt");
                }
                DapperORM dprObj = new DapperORM();
                dt = dprObj.ConvertToDataTable(GetPersonalGatePassRequest);
                ...
                worksheet.Cell(1, 1).Value = "Personal GatePass Request Report - (" + FromDate.ToString("dd/MMM/yyyy") + " - " + ToDate.ToString("dd/MMM/yyyy") + ")";
