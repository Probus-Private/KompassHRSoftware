...
{
    public class Reports_Attendance_PunchAdjustmentsController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: Reports/Reports_Attendance_PunchAdjustments
        #region PunchAdjustments
        public ActionResult Reports_Attendance_PunchAdjustments(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 375;
              ...
              //public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate, int? ContractorId)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("PunchAdjustmentsReport");
                ...
                if (CmpId != null)
                {
                    ...
                    {
                        Query = Query + " and BranchId in (select mas_branch.BranchID as Id from UserBranchMapping,mas_branch where  UserBranchMapping.employeeid = " + Session["EmployeeId"] + " and mas_branch.BranchId = UserBranchMapping.branchid and mas_branch.Deactivate = 0 and mas_branch.CmpId = " + CmpId + " and UserBranchMapping.IsActive = 1)";
                    }
                    if (EmployeeID != null)
                    {
                    ...
                    //{
                    //    Query = Query + " and CONVERT(date, DeviceLogs.LogDate) BETWEEN  '" + GetFromDate + "' AND '" + GetToDate + "' ";
                    //}
                }
                ...
                var GetPunchAdjust = DapperORM.ReturnList<dynamic>("sp_Rpt_Atten_PunchAdjustments", paramList).ToList();
                if (GetPunchAdjust.Count == 0)
                {
                    byte[] emptyFileContents = new byte[0];
                    return File(emptyFileContents, "application/octet-stream", "FileNotFound.txt");
                }
                DapperORM dprObj = new DapperORM();
                dt = dprObj.ConvertToDataTable(GetPunchAdjust);
                ...
                worksheet.Cell(1, 1).Value = "Punch Adjustments Report - (" + FromDate.ToString("dd/MMM/yyyy") + " - " + ToDate.ToString("dd/MMM/yyyy") + ")";
