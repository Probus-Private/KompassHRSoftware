...
{
    public class Reports_Attendance_DurationBasedController : Controller
    {
        // GET: Reports/Reports_Attendance_DurationBased
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        DynamicParameters param = new DynamicParameters();
        #region DurationBased
        public ActionResult Reports_Attendance_DurationBased(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 374;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate,int? ContractorId)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("DurationBasedReport");
                ...
                var GetDailyAttendanceReport = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Atten_DuartionVSShortHrs", paramList).ToList();
                ...
                worksheet.Cell(1, 1).Value = "Attendance Duration Based Report - (" + FromDate.ToString("dd/MMM/yyyy") + " - " + ToDate.ToString("dd/MMM/yyyy") + ")";
