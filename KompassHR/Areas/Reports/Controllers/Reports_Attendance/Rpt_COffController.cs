...
{
    public class Rpt_COffController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: Reports/Rpt_COff
        public ActionResult Rpt_COff(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 332;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate,int? ContractorId)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("COffReport");
                ...
                if (CmpId != null)
                {
                    ...
                    if (FromDate != null)
                    {
                        Query = Query + " and CONVERT(date, Atten_CoffGeneration.CoffGenerationDate) BETWEEN  '" + GetFromDate + "' AND '" + GetToDate + "'";
                    }
                }
                DynamicParameters paramList = new DynamicParameters();
                paramList.Add("@P_Qry", Query);
                data = DapperORM.ReturnList<dynamic>("sp_Rpt_Atten_GeneratedCoff", paramList).ToList();
                if (data.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Coff Report - (" + FromDate.ToString("dd/MMM/yyyy") + " To " + ToDate.ToString("dd/MMM/yyyy") + ")";
