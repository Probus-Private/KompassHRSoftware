...
{
    public class Rpt_Daily_AttendanceController : Controller
    {
        ...
        #region Main View
        public ActionResult Rpt_Daily_Attendance(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 264;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate,int? ContractorId)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("DailyAttendanceReport");
                ...
                var GetDailyAttendanceReport = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Atten_DailyAttendance", paramList).ToList();
        ...
        #region Preview GetDailyAttendanceReport
        public ActionResult GetDailyAttendanceReport(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate, int? ContractorId)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                DynamicParameters paramList = new DynamicParameters();
                paramList.Add("@P_Qry", Query);
                var Title = "Daily Attendance Report - (" + FromDate.ToString("dd/MMM/yyyy") + " - " + ToDate.ToString("dd/MMM/yyyy") + ")";
                var result = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Atten_DailyAttendance", paramList).ToList();
                if (result.Count == 0)
                ...
                var normalized = result.Select(r => (IDictionary<string, object>)r).ToList();

                return Json(new { success = true, data = normalized , Title = Title }, JsonRequestBehavior.AllowGet);
            }
            catch (Exception ex)
            {
                Session["GetErrorMessage"] = ex.Message;
