...
{
    public class Rpt_Attendance_EarlyGoingController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: Reports/Rpt_Attendance_EarlyGoing
        #region Rpt_Attendance_EarlyGoing
        public ActionResult Rpt_Attendance_EarlyGoing(DailyAttendanceReportFilter MasReport)
        {
            ...
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 273;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate,int? ContractorId)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("EarlyGoingReport");
                ...
                var GetEarlyGoing = DapperORM.ReturnList<dynamic>("sp_Rpt_Atten_EarlyGoing", paramList).ToList();
                if (GetEarlyGoing.Count == 0)
                {
                    byte[] emptyFileContents = new byte[0];
                    return File(emptyFileContents, "application/octet-stream", "FileNotFound.txt");
                }
                DapperORM dprObj = new DapperORM();
                dt = dprObj.ConvertToDataTable(GetEarlyGoing);
                ...
                worksheet.Cell(1, 1).Value = "Early Going Report - (" + FromDate.ToString("dd/MMM/yyyy") + " - " + ToDate.ToString("dd/MMM/yyyy") + ")";
