...
{
    public class Rpt_Attendance_MusterRoll_WithOTController : Controller
    {
        // GET: Reports/Rpt_Attendance_MusterRoll_WithOT
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        #region Main View
        public ActionResult Rpt_Attendance_MusterRoll_WithOT(DailyAttendanceReportFilter Obj)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 269;
        ...
        public ActionResult DownloadExcelFile(int? BranchId, DateTime FromDate, int CmpId, int? ContractorId)
        {
            try
            {
                ...
                foreach (var branch in selectedBranches)
                {
                    ...
                    data = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Atten_MusterRollWithOT", param).ToList();
                    ...
                    var range = worksheet.Range("U3:AY" + worksheet.LastRowUsed().RowNumber());
