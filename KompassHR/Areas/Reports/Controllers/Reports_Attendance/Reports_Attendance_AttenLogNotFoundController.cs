...
{
    public class Reports_Attendance_AttenLogNotFoundController : Controller
    {
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        DynamicParameters param = new DynamicParameters();
        // GET: Reports/Reports_Attendance_AttenLogNotFound
        #region AttenLogNotFound
        public ActionResult Reports_Attendance_AttenLogNotFound(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 260;
                ...
                paramEmpName.Add("@query", "select EmployeeId as Id, Concat(EmployeeName ,' - ', EmployeeNo) as Name  from Mas_Employee where Deactivate=0 and CmpID= " + CmpId + " and Mas_Employee.EmployeeLeft=0 order by Name");
        ...
        public ActionResult GetBusinessUnit(int CmpId)
        {
            try
            {
                ...
                DynamicParameters paramEmpName = new DynamicParameters();
                paramEmpName.Add("@query", "select EmployeeId as Id, Concat(EmployeeName ,' - ', EmployeeNo) as Name  from Mas_Employee where Deactivate=0 and EmployeeId in (Select distinct Atten_InOut.Inoutemployeeid from Atten_InOut  where deactivate=0 and Atten_InOut.InOutBranchId in (Select BranchID from UserBranchMapping where UserBranchMapping.EmployeeID=" + Session["EmployeeId"] + " and UserBranchMapping.IsActive=1 and UserBranchMapping.CmpID=" + CmpId + ") and EmployeeId<>1) union select EmployeeId as Id, Concat(EmployeeName ,' - ', EmployeeNo) as Name  from mas_employee where deactivate=0 and employeebranchid in (Select BranchID from UserBranchMapping where UserBranchMapping.EmployeeID=" + Session["EmployeeId"] + " and UserBranchMapping.IsActive=1 and UserBranchMapping.CmpID=" + CmpId + ") or mas_employee.LeavingDate is null order by Name");
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, int? GradeId, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID)
        {
            try
            {
                var workbook = new XLWorkbook();
                var worksheet = workbook.Worksheets.Add("ActiveEmployeeAttendanceMissing");
                ...
                data = DapperORM.ExecuteSP<dynamic>("SP_Rpt_Atten_LogNotFound", paramList).ToList();
                if (data.Count == 0)
                {
                ...
                // Set the header row name  var a = DateTime.Now.Date.ToString("dd/MMM/yyyy");
                worksheet.Cell(1, 1).Value = "Active Employee But Attendance Missing Report- (" + DateTime.Now.Date.ToString("dd/MMM/yyyy") + ")";
