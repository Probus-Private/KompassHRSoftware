...
{
    public class Rpt_LastDayController : Controller
    {
        // GET: Reports/Rpt_LastDay
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        DynamicParameters param = new DynamicParameters();
        #region Rpt_LastDay
        public ActionResult Rpt_LastDay(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 340;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate,int? ContractorId)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("EmployeeLastDayReport");
                ...
                if (CmpId != null)
                {
                    ...
                    {
                        Query = "and mas_branch.BranchId in (select BranchID as Id from UserBranchMapping where  UserBranchMapping.employeeid = " + Session["EmployeeId"] + " and UserBranchMapping.CmpID = " + CmpId + " and UserBranchMapping.IsActive = 1)";
                    }
                    ...
                    {
                        Query = Query + " and CONVERT(date,Mas_Employee.leavingdate) BETWEEN  '" + GetFromDate + "' AND '" + GetToDate + "' ";
                    }
                }
                DynamicParameters paramList = new DynamicParameters();
                paramList.Add("@P_Qry", Query);
                var GetLastDayReport = DapperORM.ReturnList<dynamic>("sp_Rpt_Atten_EmployeeLastWorkingDay", paramList).ToList();
                ...
                worksheet.Cell(1, 1).Value = "Employee Last Day Report - (" + FromDate.ToString("dd/MMM/yyyy") + " - " + ToDate.ToString("dd/MMM/yyyy") + ")";
