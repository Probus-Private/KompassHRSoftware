...
{
    public class Reports_Attendance_ContinuousPresentController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: Reports/Reports_Attendance_ContinuousPresent
        #region Reports_Attendance_ContinuousPresent
        public ActionResult Reports_Attendance_ContinuousPresent(DailyAttendanceReportFilter MasReport)
        {
            try
            {
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate, string MaxNoOfDays, string MinNoOfDays)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                var worksheet = workbook.Worksheets.Add("ContinuousPresentReport");
                ...
                var GetContinuousPresentReport = DapperORM.ReturnList<dynamic>("sp_Rpt_Atten_ContinuousPresent", paramList).ToList();
                if (GetContinuousPresentReport.Count == 0)
                {
                    byte[] emptyFileContents = new byte[0];
                    return File(emptyFileContents, "application/octet-stream", "FileNotFound.txt");
                }
                DapperORM dprObj = new DapperORM();
                dt = dprObj.ConvertToDataTable(GetContinuousPresentReport);
                ...
                worksheet.Cell(1, 1).Value = "Continuous Present Report - (" + FromDate.ToString("dd/MMM/yyyy") + " - " + ToDate.ToString("dd/MMM/yyyy") + ")";
