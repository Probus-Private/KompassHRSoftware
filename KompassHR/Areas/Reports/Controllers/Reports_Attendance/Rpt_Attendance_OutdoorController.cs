...
{
    public class Rpt_Attendance_OutdoorController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: Reports/Rpt_Attendance_Outdoor
        #region Rpt_Attendance_Outdoor
        public ActionResult Rpt_Attendance_Outdoor(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 271;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("OutDoor");
                ...
                if (CmpId != null)
                {
                    ...
                    if (FromDate != null)
                    {
                        Query = Query + " and convert (date,Atten_OutDoorCompany.FromDate) >=convert(date,'" + GetFromDate + "') and  convert(date,Atten_OutDoorCompany.ToDate) <=convert(date,'" + GetToDate + "')";
                    }
                }
                DynamicParameters paramList = new DynamicParameters();
                paramList.Add("@P_Qry", Query);
                data = DapperORM.ReturnList<dynamic>("sp_Rpt_Atten_ESS_OutDoorCompany", paramList).ToList();
                if (data.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Outdoor Report - (" + FromDate.ToString("dd/MMM/yyyy") + " To " + ToDate.ToString("dd/MMM/yyyy") + ")";
