...
{
    public class Rpt_Attendance_LateMarkAdjustmentController : Controller
    {
        // GET: Reports/Rpt_Attendance_LateMarkAdjustment

        #region Main View
        public ActionResult Rpt_Attendance_LateMarkAdjustment()
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 743;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime Month, DateTime ToMonth)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                var worksheet = workbook.Worksheets.Add("LateMarkAdjustmentReport");
                ...
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Atten_LateMarkAdjustment", paramList).ToList();
                if (GetData.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Late Mark Adjustment Report";
