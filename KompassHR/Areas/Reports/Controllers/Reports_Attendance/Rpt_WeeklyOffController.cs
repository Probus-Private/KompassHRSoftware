...
{
    public class Rpt_WeeklyOffController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: Reports/Rpt_WeeklyOff
        #region Rpt_WeeklyOff
        public ActionResult Rpt_WeeklyOff(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 240;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate, string Type,int? ContractorId)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("WeeklyOff");
                ...
                if (CmpId != null)
                {
                    ...
                    if (Type == "")
                    {
                        Query = Query + " and  Atten_InOut.InOutStatus in ('WO','WO(P)') ";
                ...
                worksheet.Cell(1, 1).Value = "WeekOff Report - (" + FromDate.ToString("dd/MMM/yyyy") + " To " + ToDate.ToString("dd/MMM/yyyy") + ")";
