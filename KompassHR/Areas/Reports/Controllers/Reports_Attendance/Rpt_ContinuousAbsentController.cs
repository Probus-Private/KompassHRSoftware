...
{
    public class Rpt_ContinuousAbsentController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: Reports/Rpt_ContinuousAbsent
        #region Rpt_ContinuousAbsent
        public ActionResult Rpt_ContinuousAbsent(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 313;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate, string MinNoOfDays, string MaxNoOfDays/*,int? ContractorId*/)
        {
            try
            {
                var workbook = new XLWorkbook();
                var worksheet = workbook.Worksheets.Add("ContinuousAbsent");
                ...
                if (CmpId != null)
                {
                    Query = " where CmpId=" + CmpId + "";
                    if (MinNoOfDays != null)
                    {
                        Query = Query + " and noofdays between " + MinNoOfDays + " and " + MaxNoOfDays + "";
                    }
                    if (BranchId != null)
                    {
                ...
                //Query = Query + "  ORDER BY BranchName,EmployeeName,InOutDate";
                ...
                data = DapperORM.ReturnList<dynamic>("sp_Rpt_Atten_ContinuousAbsent", paramList).ToList();
                if (data.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "ContinuousAbsent Report - (" + FromDate.ToString("dd/MMM/yyyy") + " To " + ToDate.ToString("dd/MMM/yyyy") + ")";
