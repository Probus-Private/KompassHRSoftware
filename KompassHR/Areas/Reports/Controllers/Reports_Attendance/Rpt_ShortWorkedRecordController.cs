...
{
    public class Rpt_ShortWorkedRecordController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: Reports/Rpt_ShortWorkedRecord
        #region Rpt_ShortWorkedRecord
        public ActionResult Rpt_ShortWorkedRecord(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 330;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate, int MinHours, int MaxHours,int? ContractorId)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                var worksheet = workbook.Worksheets.Add("ShortWorkedHrsRecordReport");
                ...
                if (CmpId != null)
                {
                    ...
                    var GetMinHrs = MinHours * 60;

                    if (MinHours == 0)
                    {
                        GetMinHrs = 1;
                    }
                    var GetMaxHrs = MaxHours * 60;

                    if (GetMinHrs != null && GetMaxHrs != null)
                    {
                        //CONVERT(date, Atten_InOut.InOutDate, 106)
                        Query = Query + "and (InOutShiftDuration-Atten_InOut.InOutDuration) between  '" + GetMinHrs + "' and '" + GetMaxHrs + "' ";
                    }
                    if (BranchId != null)
                    {
                ...
                var ShortWorkedRecord = DapperORM.ReturnList<dynamic>("sp_Rpt_Atten_ShortWorkHrsRecord", paramList).ToList();
                if (ShortWorkedRecord.Count == 0)
                {
                    byte[] emptyFileContents = new byte[0];
                    return File(emptyFileContents, "application/octet-stream", "FileNotFound.txt");
                }
                DapperORM dprObj = new DapperORM();
                dt = dprObj.ConvertToDataTable(ShortWorkedRecord);
                ...
                worksheet.Cell(1, 1).Value = "Short Work HrsRecord Report - (" + FromDate.ToString("dd/MMM/yyyy") + " - " + ToDate.ToString("dd/MMM/yyyy") + ")";
