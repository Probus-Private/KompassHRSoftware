...
{
    public class Reports_Attendance_TotalEmployeeAbsentController : Controller
    {
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        DynamicParameters param = new DynamicParameters();
        // GET: Reports/Reports_Attendance_TotalEmployeeAbsent
        #region Reports_Attendance_TotalEmployeeAbsent
        public ActionResult Reports_Attendance_TotalEmployeeAbsent(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 308;
                ...
                ViewBag.TotalEmployeeAbsentReport = "";
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate,int? ContractorId)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("TotalEmployeeAbsent");
                worksheet.Range(1, 1, 1, 36).Merge();
                ...
                data = DapperORM.ReturnList<dynamic>("sp_Rpt_Atten_TotalEmployeeAbsent", paramList).ToList();
                if (data.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Total Employee Absent Report - (" + FromDate.ToString("dd/MMM/yyyy") + " To " + ToDate.ToString("dd/MMM/yyyy") + ")";
