...
{
    public class Reports_Attendance_TotalEmployeePresentController : Controller
    {
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        DynamicParameters param = new DynamicParameters();
        // GET: Reports/Reports_Attendance_TotalEmployeePresent
        #region Reports_Attendance_TotalEmployeePresent
        public ActionResult Reports_Attendance_TotalEmployeePresent(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 309;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate,int? ContractorId)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("TotalEmployeePresentReport");
                ...
                //{
                //    Query = Query + " and CONVERT(date, Atten_InOut.InOutDate, 106) BETWEEN  '" + MasReport.FromDate + "' AND '" + MasReport.ToDate + "' ";
                //}
                DynamicParameters paramList = new DynamicParameters();
                paramList.Add("@P_Qry", Query);
                var GetTotalEmployeePresent = DapperORM.ReturnList<dynamic>("sp_Rpt_Atten_TotalEmployeePresent", paramList).ToList();
                if (GetTotalEmployeePresent.Count == 0)
                {
                    byte[] emptyFileContents = new byte[0];
                    return File(emptyFileContents, "application/octet-stream", "FileNotFound.txt");
                }
                DapperORM dprObj = new DapperORM();
                dt = dprObj.ConvertToDataTable(GetTotalEmployeePresent);
                ...
                worksheet.Cell(1, 1).Value = "Total Employee Present Report - (" + FromDate.ToString("dd/MMM/yyyy") + " - " + ToDate.ToString("dd/MMM/yyyy") + ")";
