...
{
    public class Rpt_ShiftWiseController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: Reports/Rpt_ShiftWise
        public ActionResult Rpt_ShiftWise(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 328;
                ...
                ViewBag.ShiftSortName = DapperORM.ReturnList<ShiftDropDown>("sp_QueryExcution", paramshift).ToList();
        ...
        //        DynamicParameters paramshift = new DynamicParameters();
        //        paramshift.Add("@query", "select distinct ShiftName +(+' ( '+format(convert(datetime,BeginTime),'HH:mm')+' - '+format(convert(datetime,EndTime),'HH:mm')+')' ) as Name ,SName  from Atten_Shifts  where Atten_Shifts.Deactivate=0 and  CmpId=" + CmpId + "");
        ...
        //        return Json(new { Branch = Branch, GetShiftSortName = GetShiftSortName, EmployeeName = EmployeeName }, JsonRequestBehavior.AllowGet);
        //    }
        //    catch (Exception ex)
        //    {
        //        Session["GetErrorMessage"] = ex.Message;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate, string ShiftSortName,int? ContractorId)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                var worksheet = workbook.Worksheets.Add("ShiftWiseReport");
                worksheet.Range(1, 1, 1, 28).Merge();
                ...
                if (CmpId != null)
                {
                    ...
                    if (ShiftSortName != "")
                    {
                        Query = Query + " and Atten_InOut.InOutShift='" + ShiftSortName + "'";
                    }
                    else
                    {
                        Query = Query + " and Atten_InOut.InOutShift in (select distinct Sname from atten_shifts where deactivate=0)";
                    }
                    if (BranchId != null)
                    {
                ...
                //{
                //    Query = Query + " and CONVERT(date, Atten_InOut.InOutDate) BETWEEN  '" + GetFromDate + "' AND '" + GetToDate + "' and Mas_employee.JoiningDate<='" + GetFromDate + "' and (Mas_employee.LeavingDate>='" + GetFromDate + "' or Mas_employee.LeavingDate is null) ";
                //}
                DynamicParameters paramList = new DynamicParameters();
                paramList.Add("@P_Qry", Query);
                var GetShiftWise = DapperORM.ReturnList<dynamic>("sp_Rpt_Atten_ShiftWise", paramList).ToList();
                if (GetShiftWise.Count == 0)
                {
                    byte[] emptyFileContents = new byte[0];
                    return File(emptyFileContents, "application/octet-stream", "FileNotFound.txt");
                }
                DapperORM dprObj = new DapperORM();
                dt = dprObj.ConvertToDataTable(GetShiftWise);
                ...
                worksheet.Cell(1, 1).Value = "Shift Wise Report - (" + FromDate.ToString("dd/MMM/yyyy") + " - " + ToDate.ToString("dd/MMM/yyyy") + ")";
