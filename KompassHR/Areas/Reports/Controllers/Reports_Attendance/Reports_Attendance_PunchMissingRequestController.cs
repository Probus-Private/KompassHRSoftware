...
{
    public class Reports_Attendance_PunchMissingRequestController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: Reports/Reports_Attendance_PunchMissingRequest
        #region PunchMissingRequest
        public ActionResult Reports_Attendance_PunchMissingRequest(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 367;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("PunchMissingRequestReport");
                ...
                if (CmpId != null)
                {
                    Query = " and Atten_PunchMissing.CmpId=" + CmpId + "";
                    ...
                    {
                        Query = Query + " and CONVERT(date, Atten_PunchMissing.PunchMissingAttendanceDate) BETWEEN  '" + GetFromDate + "' AND '" + GetToDate + "' ";
                    }
                }
                DynamicParameters paramList = new DynamicParameters();
                paramList.Add("@P_Qry", Query);
                var GetPunchMissingRequest = DapperORM.ReturnList<dynamic>("sp_Rpt_Atten_ESS_PunchMissing", paramList).ToList();
                if (GetPunchMissingRequest.Count == 0)
                {
                    byte[] emptyFileContents = new byte[0];
                    return File(emptyFileContents, "application/octet-stream", "FileNotFound.txt");
                }
                DapperORM dprObj = new DapperORM();
                dt = dprObj.ConvertToDataTable(GetPunchMissingRequest);
                ...
                worksheet.Cell(1, 1).Value = "Punch Missing Request Report - (" + FromDate.ToString("dd/MMM/yyyy") + " - " + ToDate.ToString("dd/MMM/yyyy") + ")";
