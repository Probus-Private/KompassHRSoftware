...
{
    public class Reports_Leave_TotalEmployeeOnLeaveController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: Reports/Reports_Leave_TotalEmployeeOnLeave
        #region TotalEmployeeOnLeave
        public ActionResult Reports_Leave_TotalEmployeeOnLeave(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 311;
                ...
                //    var TotalEmployeeOnLeave = DapperORM.ReturnList<dynamic>("sp_Rpt_Leave_TotalEmployeeOnLeave", paramList).ToList();
                //    ViewBag.TotalEmployeeOnLeave = TotalEmployeeOnLeave;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("TotalEmployeeOnLeaveReport");
                ...
                if (CmpId != null)
                {
                    ...
                    {
                        Query = Query + " and and Mas_Branch.BranchId in (select UserBranchMapping.BranchID as Id from UserBranchMapping where  UserBranchMapping.employeeid = " + Session["EmployeeId"] + "  and UserBranchMapping.CmpID = " + CmpId + " and UserBranchMapping.IsActive = 1 union select EmployeeBranchId as Id  from Mas_Employee where  EmployeeId= " + Session["EmployeeId"] + "  and Mas_Employee.CmpID= " + CmpId + " )";
                    }
                ...
                paramList.Add("@p_Msg", dbType: DbType.String, direction: ParameterDirection.Output, size: 500);//OutPut Parameter
                var TotalEmployeeOnLeave = DapperORM.ReturnList<dynamic>("sp_Rpt_Leave_TotalEmployeeOnLeave", paramList).ToList();
                if (TotalEmployeeOnLeave.Count == 0)
                {
                    byte[] emptyFileContents = new byte[0];
                    return File(emptyFileContents, "application/octet-stream", "FileNotFound.txt");
                }
                DapperORM dprObj = new DapperORM();
                dt = dprObj.ConvertToDataTable(TotalEmployeeOnLeave);
                ...
                worksheet.Cell(1, 1).Value = "Total Employee On Leave Report- (" + FromDate.ToString("dd/MMM/yyyy") + " - " + ToDate.ToString("dd/MMM/yyyy") + ")";
