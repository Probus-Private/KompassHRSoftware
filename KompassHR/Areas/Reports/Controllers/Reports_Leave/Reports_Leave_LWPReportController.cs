...
{
    public class Reports_Leave_LWPReportController : Controller
    {
        // GET: Reports/Reports_Leave_LWPReport
        #region Main view
        public ActionResult Reports_Leave_LWPReport()
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 737;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime Month)
        {
            try
            {
                ...
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Leave_WithoutPay_Report", paramList).ToList();
                if (GetData.Count == 0)
                {
