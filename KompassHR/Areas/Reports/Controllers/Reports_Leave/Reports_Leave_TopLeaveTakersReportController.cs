...
{
    public class Reports_Leave_TopLeaveTakersReportController : Controller
    {
        // GET: Reports/Rpt_Payroll_BranchUnitWiseSummary
        public ActionResult Reports_Leave_TopLeaveTakersReport(RptLeaveSummery RptLeaveSummery)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 733;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, int? LeaveYear)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("TopLeaveTakersReport");
                ...
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Leave_TopLeaveTakerReport", paramList).ToList();
                if (GetData.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Top Leave Takers Report";
