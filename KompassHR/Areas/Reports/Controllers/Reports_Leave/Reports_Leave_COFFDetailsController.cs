...
{
    public class Reports_Leave_COFFDetailsController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: Reports/Reports_Leave_COFFDetails
        #region Reports_Leave_COFFDetails
        public ActionResult Reports_Leave_COFFDetails(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 321;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("COFFDetailsReport");
                ...
                if (CmpId != null)
                {
                    ...
                    {
                        Query = Query + " and Mas_SubDepartment.SubDepartementId=" + SubDepartmentId + "";
                    }
                    ...
                    if (FromDate != null)
                    {
                        Query = Query + " and (Atten_CoffRequest.FromDate between '" + GetFromDate + "' and '" + GetToDate + "' or Atten_CoffRequest.ToDate between '" + GetFromDate + "' and '" + GetToDate + "' )";
                    }
                }
                DynamicParameters paramList = new DynamicParameters();
                paramList.Add("@P_Qry", Query);
                var COFFDetails = DapperORM.ReturnList<dynamic>("sp_Rpt_Leave_COFFDetails", paramList).ToList();
                ...
                worksheet.Cell(1, 1).Value = "COFF Details Report Report- (" + FromDate.ToString("dd/MMM/yyyy") + " - " + ToDate.ToString("dd/MMM/yyyy") + ")";
