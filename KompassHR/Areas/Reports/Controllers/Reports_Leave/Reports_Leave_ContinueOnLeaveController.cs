...
{
    public class Reports_Leave_ContinueOnLeaveController : Controller
    {
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        DynamicParameters param = new DynamicParameters();
        // GET: Reports/Reports_Leave_ContinueOnLeave

        #region Reports_Leave_ContinueOnLeave
        public ActionResult Reports_Leave_ContinueOnLeave(DailyAttendanceReportFilter MasReport) //DailyAttendanceReportFilter OBJContinueOnLeave
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 300;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeId, int? GradeId, DateTime ToDate)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("ContinueOnLeaveReport");
                ...
                if (CmpId != null)
                {
                    ...
                    {
                        Query = " where Mas_Branch.BranchId in (select UserBranchMapping.BranchID as Id from UserBranchMapping where  UserBranchMapping.employeeid = " + Session["EmployeeId"] + "  and UserBranchMapping.CmpID = " + CmpId + " and UserBranchMapping.IsActive = 1 union select EmployeeBranchId as Id  from Mas_Employee where  EmployeeId= " + Session["EmployeeId"] + "  and Mas_Employee.CmpID= " + CmpId + " )";
                    }
                    ...
                    if (FromDate != null)
                    {
                        Query = Query + " and convert (date,Leave_Master.FromDate) >=convert(date,'" + GetFromDate + "') and  convert(date,Leave_Master.ToDate) <=convert(date,'" + GetToDate + "') order by FromDate";
                    }
                }

                DynamicParameters paramList = new DynamicParameters();
                paramList.Add("@P_Qry", Query);
                var ContinueOnLeave = DapperORM.ReturnList<dynamic>("sp_Rpt_Leave_ContinueOnLeave", paramList).ToList();
                if (ContinueOnLeave.Count == 0)
                {
                    byte[] emptyFileContents = new byte[0];
                    return File(emptyFileContents, "application/octet-stream", "FileNotFound.txt");
                }
                DapperORM dprObj = new DapperORM();
                dt = dprObj.ConvertToDataTable(ContinueOnLeave);
                ...
                worksheet.Cell(1, 1).Value = "Continue On Leave Report - (" + FromDate.ToString("dd/MMM/yyyy") + " - " + ToDate.ToString("dd/MMM/yyyy") + ")";
