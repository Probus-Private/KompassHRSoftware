...
{
    public class Reports_Leave_ZeroLeaveTakersReportController : Controller
    {
        // GET: Reports/Rpt_Payroll_BranchUnitWiseSummary
        public ActionResult Reports_Leave_ZeroLeaveTakersReport(RptLeaveSummery RptLeaveSummery)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 734;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, int? LeaveYear)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("ZeroLeaveTakersReport");
                ...
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Leave_ZeroLeaveTakerReport", paramList).ToList();
                if (GetData.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "Zero Leave Takers Report";
