...
{
    public class Reports_Leave_MaternityPaternityLeaveReportController : Controller
    {
        // GET: Reports/Rpt_Payroll_BranchUnitWiseSummary
        public ActionResult Reports_Leave_MaternityPaternityLeaveReport(RptLeaveSummery RptLeaveSummery)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 735;
                ...
                param.Add("@query", "SELECT LeaveTypeId as Id,LeaveTypeShortName as Name FROM Leave_Type,Leave_Setting where Leave_Type.Deactivate = 0 and LeaveTypeId=4 and Leave_Setting.Deactivate=0 and Leave_Type.LeaveTypeId=Leave_Setting.LeaveSettingLeaveTypeId and Leave_Setting.LeaveSettingLeaveGroupId=(Select  EM_Atten_LeaveGroupID from Mas_Employee_Attendance where AttendanceEmployeeId=" + Session["EmployeeId"] + " )");
                var GetLeaveType = DapperORM.ReturnList<AllDropDownBind>("sp_QueryExcution", param).ToList();
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, int? LeaveType,int? LeaveYear)
        {
            try
            {
                if (Session["EmployeeId"] == null)
                ...
                var worksheet = workbook.Worksheets.Add("MaternityPaternityLeaveReport");
                ...
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Rpt_Leave_MaternityPaternityLeaveReport", paramList).ToList();
                if (GetData.Count == 0)
                {
                ...
                worksheet.Cell(1, 1).Value = "MaternityPaternityLeaveReport";
