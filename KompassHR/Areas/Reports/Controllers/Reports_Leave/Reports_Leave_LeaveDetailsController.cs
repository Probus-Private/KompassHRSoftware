...
{
    public class Reports_Leave_LeaveDetailsController : Controller
    {
        DynamicParameters param = new DynamicParameters();
        SqlConnection sqlcon = new SqlConnection(DapperORM.connectionString);
        // GET: Reports/Reports_Leave_LeaveDetails
        #region Reports_Leave_LeaveDetails
        public ActionResult Reports_Leave_LeaveDetails(DailyAttendanceReportFilter MasReport)
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 318;
                ...
                //    {
                //        Query = Query + " and FromDate between'" + GetFromDate + "' and '" + GetToDate + "'";
                //    }


                //    DynamicParameters paramList = new DynamicParameters();
                //    paramList.Add("@P_Qry", Query);
                //    var GetLeaveDetails =DapperORM.ReturnList<dynamic>("sp_List_Leave_Master", paramList).ToList();
                //    ViewBag.GetLeaveDetailsReport = GetLeaveDetails;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, DateTime FromDate, int? SubUnitId, int? DepartmentId, int? SubDepartmentId, int? DesignationId, int? LineId, int? EmployeeID, int? GradeId, DateTime ToDate)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("LeaveDetailsReport");
                ...
                if (CmpId != null)
                {
                    ...
                    if (FromDate != null)
                    {
                        Query = Query + " and (Leave_Master.FromDate between'" + GetFromDate + "' and '" + GetToDate + "' or Leave_Master.ToDate between'" + GetFromDate + "' and '" + GetToDate + "' ) order by FromDate";
                    }
                }

                DynamicParameters paramList = new DynamicParameters();
                paramList.Add("@P_Qry", Query);
                var GetLeaveDetails = DapperORM.ReturnList<dynamic>("sp_Rpt_Leave_TotalEmployeeOnLeave", paramList).ToList();
                if (GetLeaveDetails.Count == 0)
                {
                    byte[] emptyFileContents = new byte[0];
                    return File(emptyFileContents, "application/octet-stream", "FileNotFound.txt");
                }
                DapperORM dprObj = new DapperORM();
                dt = dprObj.ConvertToDataTable(GetLeaveDetails);
                ...
                worksheet.Cell(1, 1).Value = "Leave Details Report - (" + FromDate.ToString("dd/MMM/yyyy") + " - " + ToDate.ToString("dd/MMM/yyyy") + ")";
