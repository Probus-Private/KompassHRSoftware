...
{
    public class Reports_Leave_LeaveDuringNoticePeriodReportController : Controller
    {
        #region Main View
        // GET: Reports/Reports_Leave_LeaveDuringNoticePeriodReport
        public ActionResult Reports_Leave_LeaveDuringNoticePeriodReport()
        {
            try
            {
                ...
                int screenId = Request.QueryString["ScreenId"] != null ? Convert.ToInt32(Request.QueryString["ScreenId"]) : 738;
        ...
        public ActionResult DownloadExcelFile(int? CmpId, int? BranchId, int? Year)
        {
            try
            {
                ...
                var worksheet = workbook.Worksheets.Add("LeaveWithNoticePeriodReport");
                ...
                paramList.Add("@p_MonthYear", Year);
                paramList.Add("@p_CompanyId", CmpId);
                paramList.Add("@p_BranchId", BranchId);
                var GetData = DapperORM.ExecuteSP<dynamic>("sp_Leave_WithNoticePeriod_Report", paramList).ToList();
                if (GetData.Count == 0)
                {
                ...
                if (Year.HasValue)
                    ...
                    worksheet.Cell(1, 1).Value = "Leave During Notice Period Report";
                else
                    worksheet.Cell(1, 1).Value = "Leave During Notice Period Report";
